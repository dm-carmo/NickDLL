...
DWORD* bra_second_vtable = (DWORD*)0x967E4C;

char* bra_second_set_champion(BYTE* _this) {
...
void __declspec(naked) bra_second_set_champion_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_second_set_champion
		add esp, 0x4
		ret 0
	}
}

void bra_second_subs(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
	...
	comp_data->promotes_to = BRA_FIRST_9CF();
	comp_data->relegates_to = BRA_THIRD_9CF();
...
void __declspec(naked) bra_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_second_subs
		add esp, 0x4
		ret
	}
}

DWORD bra_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = None;
		...
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 4, 2), year, Wednesday, Evening);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 6, 11), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Thursday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
...
void __declspec(naked) bra_second_fixtures_c()
{
	__asm
	{
		...
		push ecx
		call bra_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void bra_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	...
	bra_second_subs(_this);
	AddTeams(_this);
	SetupTVMoney(_this, prizeMoneyFile.GetInt("bra_second_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
...
char bra_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
...
void __declspec(naked) bra_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_second_update
		add esp, 0x4
		ret
	}
}

void bra_second_prom_playoffs(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_second_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			bra_second_prom_playoffs(_this);
		}
	}
}

void __declspec(naked) bra_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_second_playoffs_c
		add esp, 0x4
		ret
	}
}
...
void bra_second_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_second_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_second_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

void setup_bra_second()
{
