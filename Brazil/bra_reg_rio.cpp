...
DWORD* bra_reg_rio_vtable = (DWORD*)0x968608;

int bra_reg_rio_set_champion(BYTE* _this) {
...
void __declspec(naked) bra_reg_rio_set_champion_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_rio_set_champion
		add esp, 0x4
		ret 0
	}
}

void bra_reg_rio_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_rio_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call bra_reg_rio_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

void bra_reg_rio_reputation_setup(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_rio_reputation_setup_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_rio_reputation_setup
		add esp, 0x4
		ret
	}
}

void bra_reg_rio_subs(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_rio_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_rio_subs
		add esp, 0x4
		ret
	}
}

DWORD bra_reg_rio_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = AlphabeticGroupStage + stage_idx;
		...
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Sunday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 2, 9), year, Sunday);
		...
		*stage_name_id = None;
		...
		int fixture_id = 0;
		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 2, 10), year, Monday);
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup | NoAwayGoals, Penalties | NoAwayGoals, 5, 4, 2, 0, 0, 0, 2, 7, prizeMoneyFile.GetInt("bra_rj_playoff_semi_money"));
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 0, Penalties, NoTiebreak, 5, 2, 1, 0, 0, 0, 1, 0, 0, prizeMoneyFile.GetInt("bra_rj_playoff_winner_money"), prizeMoneyFile.GetInt("bra_rj_playoff_runner_up_money"));
		...
		*stage_name_id = RelegationPlayoff;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 2, 26), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 1, Thursday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
...
void __declspec(naked) bra_reg_rio_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_rio_fixtures
		add esp, 0x14
		ret 0x10
	}
}

int bra_reg_rio_table_fates(BYTE* _this, cm3_clubs* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) bra_reg_rio_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_rio_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

int bra_reg_rio_add_teams(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_reg_rio_setup_groups(BYTE* _this, BYTE idx) {
	DWORD v1 = *(DWORD*)_this;
	WORD num_rounds = 0;
	...
	*((DWORD*)(&stages_arr[idx])) = (DWORD)pStage;
	SetupTVMoney(pStage, prizeMoneyFile.GetInt("bra_rj_tv_money"), 0);
	sub_9452CA_free(pTeams);
	sub_9452CA_free(pFixtures);
	data->current_stage = idx;
}

char bra_reg_rio_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	...
	bra_reg_rio_add_teams(_this);
	SetupTVMoney(_this, prizeMoneyFile.GetInt("bra_rj_tv_money"), 0);
	sub_6827D0(_this, 0);
	sub_6835C0(_this);
	for (BYTE i = 0; i < 1; i++) {
		bra_reg_rio_setup_groups(_this, i);
	}
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) bra_reg_rio_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_rio_update
		add esp, 0x4
		ret
	}
}

void bra_reg_rio_playoff_champ(BYTE* _this) {
	char stage_num = 1;
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_reg_rio_playoff_rele(BYTE* _this) {
	char stage_num = 2;
	DWORD v1 = *(DWORD*)_this;
	WORD num_rounds = 0;
	WORD stage_name_id = 0;
...
void bra_reg_rio_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 1) {
			bra_reg_rio_playoff_champ(_this);
			bra_reg_rio_playoff_rele(_this);
		}
	}
}

void __declspec(naked) bra_reg_rio_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_rio_playoffs_c
		add esp, 0x4
		ret
	}
}

void bra_reg_rio_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = bra_reg_rio_vtable;
	data->year = year;
	...
	bra_reg_rio_reputation_setup(_this);
}

void bra_reg_rio_award_teams(BYTE* _this, DWORD** team_list, WORD* total_teams) {
	*total_teams = 12;
...
void __declspec(naked) bra_reg_rio_award_teams_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_rio_award_teams
		add esp, 0xc
		ret 8
	}
}

void setup_bra_reg_rio() {
	WriteVTablePtr(bra_reg_rio_vtable, VTableSubsRounds, (DWORD)&bra_reg_rio_subs_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTablePostMatchUpdate, 0x685d30);
	WriteVTablePtr(bra_reg_rio_vtable, VTableReputationSetup, (DWORD)&bra_reg_rio_reputation_setup_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTableFixtures, (DWORD)&bra_reg_rio_fixtures_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTableEoSUpdate, (DWORD)&bra_reg_rio_update_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTableReputationCalc, (DWORD)&bra_reg_rio_reputation_calc_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTableSetChampion, (DWORD)&bra_reg_rio_set_champion_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTableTableFates, (DWORD)&bra_reg_rio_table_fates_c);
	WriteVTablePtr(bra_reg_rio_vtable, VTablePlayoffQual, (DWORD)&bra_reg_rio_playoffs_create);
	WriteVTablePtr(bra_reg_rio_vtable, VTableAwardTeamsSetup, (DWORD)&bra_reg_rio_award_teams_c);
}