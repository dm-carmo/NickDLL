...
DWORD bra_reg_rio_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 0, PenaltiesNoExtraTime_1, NoTiebreak_2, 5, 2, 1, 0, 0, 0, 1, 0, 0, prizeMoneyFile.GetInt("bra_rj_playoff_winner_money"), prizeMoneyFile.GetInt("bra_rj_playoff_runner_up_money"));
...
int bra_reg_rio_set_fates(BYTE* _this, cm3_clubs* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) bra_reg_rio_set_table_fate()
{
	__asm
	{
		mov eax, esp
...
void setup_bra_reg_rio() {
	...
	WriteVTablePtr(bra_reg_rio_vtable, VTableTableFates, (DWORD)&bra_reg_rio_set_table_fate);
