...
DWORD bra_reg_catarina_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, QuarterFinal, 0, FixedTeamOrderInCup2 + PenaltiesNoExtraTime_1, NoTiebreak_2, 5, 8, 4, 8, 0, 0, 1, 0);
...
void bra_reg_catarina_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	BYTE playoff_teams = comp_data->prom_playoff;
	WORD total_teams = comp_data->n_teams;
	DWORD* pTeams = (DWORD*)sub_944E46_malloc(playoff_teams * 4);
	BYTE team_order[8] = { 2,4,6,0,1,7,5,3 };
...
void bra_reg_catarina_playoffs_c(BYTE* _this) {
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			bra_reg_catarina_playoff_under(_this);
		}
	}
}
...
void setup_bra_reg_catarina() {
	...
	WriteVTablePtr(bra_reg_catarina_vtable, VTableReputationSetup, 0x68a850);
