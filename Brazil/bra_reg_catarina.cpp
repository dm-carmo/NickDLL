...
DWORD bra_reg_catarina_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 0, NoAwayGoals, Penalties | NoAwayGoals, 5, 2, 1, 0, 0, 0, 2, 7, 0, prizeMoneyFile.GetInt("bra_sc_playoff_winner_money"), prizeMoneyFile.GetInt("bra_sc_playoff_runner_up_money"));
...
int bra_reg_catarina_table_fates(BYTE* _this, cm3_clubs* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) bra_reg_catarina_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_catarina_table_fates
		add esp, 0x1c
		ret 0x18
	}
}
...
void setup_bra_reg_catarina() {
	...
	WriteVTablePtr(bra_reg_catarina_vtable, VTableTableFates, (DWORD)&bra_reg_catarina_table_fates_c);
