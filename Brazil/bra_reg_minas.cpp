...
DWORD* bra_reg_minas_vtable = (DWORD*)0x968284;

int bra_reg_minas_set_champion(BYTE* _this) {
...
void __declspec(naked) bra_reg_minas_set_champion_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_minas_set_champion
		add esp, 0x4
		ret 0
	}
}

void bra_reg_minas_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_minas_reputation_calc_c()
{
	__asm
	{
		...
		push ecx
		call bra_reg_minas_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

void bra_reg_minas_reputation_setup(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_minas_reputation_setup_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_minas_reputation_setup
		add esp, 0x4
		ret
	}
}

void bra_reg_minas_subs(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_minas_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_minas_subs
		add esp, 0x4
		ret
	}
}

DWORD bra_reg_minas_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 1) {
		if (a5)
			*a5 = 1;
...
int bra_reg_minas_add_teams(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_reg_minas_setup_groups(BYTE* _this, BYTE idx) {
	DWORD v1 = *(DWORD*)_this;
	WORD num_rounds = 0;
	...
	*((DWORD*)(&stages_arr[idx])) = (DWORD)pStage;
	SetupTVMoney(pStage, prizeMoneyFile.GetInt("bra_mg_tv_money"), 0);
	sub_9452CA_free(pTeams);
	sub_9452CA_free(pFixtures);
	data->current_stage = idx;
}

char bra_reg_minas_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
	...
	bra_reg_minas_add_teams(_this);
	SetupTVMoney(_this, prizeMoneyFile.GetInt("bra_mg_tv_money"), 0);
	BYTE* edx = 0;
	sub_6827D0(_this, edx);
	sub_6835C0(_this);
	for (BYTE i = 0; i < 1; i++) {
		bra_reg_minas_setup_groups(_this, i);
	}
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) bra_reg_minas_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_minas_update
		add esp, 0x4
		ret
	}
}

void bra_reg_minas_playoff_champ(BYTE* _this) {
	char stage_num = 1;
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_reg_minas_playoff_rele(BYTE* _this) {
	char stage_num = 2;
	DWORD v1 = *(DWORD*)_this;
	WORD num_rounds = 0;
	WORD stage_name_id = 0;
...
void bra_reg_minas_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 1) {
			bra_reg_minas_playoff_champ(_this);
			bra_reg_minas_playoff_rele(_this);
		}
	}
}
...
void __declspec(naked) bra_reg_minas_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_minas_playoffs_c
		add esp, 0x4
		ret
	}
}

void bra_reg_minas_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = bra_reg_minas_vtable;
	data->year = year;
	...
	bra_reg_minas_reputation_setup(_this);
}

void bra_reg_minas_award_teams(BYTE* _this, DWORD** team_list, WORD* total_teams) {
	*total_teams = 12;
...
void __declspec(naked) bra_reg_minas_award_teams_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_minas_award_teams
		add esp, 0xc
		ret 8
	}
}
...
void setup_bra_reg_minas() {
	...
	WriteVTablePtr(bra_reg_minas_vtable, VTableReputationSetup, (DWORD)&bra_reg_minas_reputation_setup_c);
