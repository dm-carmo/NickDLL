...
int bra_reg_minas_set_fates(BYTE* _this, DWORD* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void bra_reg_minas_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_reg_minas_playoffs_c(BYTE* _this) {
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			bra_reg_minas_playoff_under(_this);
		}
	}
}
