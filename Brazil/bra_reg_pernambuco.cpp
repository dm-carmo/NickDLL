...
void __declspec(naked) bra_reg_pernambuco_set_champion_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) bra_reg_pernambuco_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) bra_reg_pernambuco_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
DWORD bra_reg_pernambuco_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup2 + NoTiebreak_1, AwayGoalsPenaltiesNoExtraTime_2, 5, 4, 2, 2, 4, 0, 2, 7);
...
void __declspec(naked) bra_reg_pernambuco_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int bra_reg_pernambuco_set_fates(BYTE* _this, DWORD* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage < 0) {
		...
		case Promoted:
			...
			return 0;
		case TopPlayoff:
			staff_history_qualified_868DD0(staff_hist_ptr, club, (DWORD)(comp_data->competition_db), None, SecondStage, 0x1E);
			return 0;
		case BottomPlayoff:
			return 0;
		case Relegated:
...
void __declspec(naked) bra_reg_pernambuco_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_pernambuco_set_fates
		add esp, 0x1c
		ret 0x18
	}
}
...
void __declspec(naked) bra_reg_pernambuco_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) bra_reg_pernambuco_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void setup_bra_reg_pernambuco() {
	...
	WriteVTablePtr(bra_reg_pernambuco_vtable, VTableReputationSetup, (DWORD)0x68a850);
	...
	WriteVTablePtr(bra_reg_pernambuco_vtable, VTableTableFates, (DWORD)&bra_reg_pernambuco_set_table_fate);
