...
DWORD* bra_reg_north_vtable = (DWORD*)0x968338;

int bra_reg_north_set_champion(BYTE* _this) {
...
void __declspec(naked) bra_reg_north_set_champion_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_north_set_champion
		add esp, 0x4
		ret 0
	}
}

void bra_reg_north_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_north_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call bra_reg_north_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

void bra_reg_north_subs(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) bra_reg_north_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_north_subs
		add esp, 0x4
		ret
	}
}

DWORD bra_reg_north_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) bra_reg_north_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bra_reg_north_fixtures
		add esp, 0x14
		ret 0x10
	}
}

int bra_reg_north_set_fates(BYTE* _this, DWORD* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) bra_reg_north_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
char bra_reg_north_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
...
void __declspec(naked) bra_reg_north_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_north_update
		add esp, 0x4
		ret
	}
}

void bra_reg_north_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
...
void bra_reg_north_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			bra_reg_north_playoff_under(_this);
		}
	}
}

void __declspec(naked) bra_reg_north_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call bra_reg_north_playoffs_c
		add esp, 0x4
		ret
	}
}

void bra_reg_north_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
...
void setup_bra_reg_north() {
	WriteVTablePtr(bra_reg_north_vtable, VTableSubsRounds, (DWORD)&bra_reg_north_subs_c);
	WriteVTablePtr(bra_reg_north_vtable, VTableReputationSetup, (DWORD)0x68a850);
	WriteVTablePtr(bra_reg_north_vtable, VTableFixtures, (DWORD)&bra_reg_north_fixtures_c);
