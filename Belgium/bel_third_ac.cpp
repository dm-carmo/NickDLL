...
vtable* bel_third_ac_vtable = new vtable((BYTE*)0x969798, 0xB4);

void bel_third_ac_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(bel_third_ac_vtable->vtable_ptr);
	DWORD x = 0;
	sub_687970(_this, 0);
...
void bel_third_ac_free(BYTE* _this, BYTE a2) {
	bel_third_ac_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) bel_third_ac_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call bel_third_ac_free
		add esp, 0x8
		ret 4
	}
}

int bel_third_ac_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 6;
...
DWORD bel_third_ac_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = None;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year + 1, 5, 3), year, Saturday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 1, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Sunday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		AddFixtureNoTV(pMem, fixture_id++, Date(year + 1, 5, 10), year, Saturday);
		...
		*stage_name_id = ChampionshipGroup + stage_idx;
		...
		AddFixtureNoTV(pMem, fixture_id++, Date(year + 1, 5, 3), year, Saturday);
		AddFixtureNoTV(pMem, fixture_id++, Date(year + 1, 5, 10), year, Saturday);
...
void __declspec(naked) bel_third_ac_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bel_third_ac_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void bel_third_ac_subs(BYTE* _this)
{
	...
	*((DWORD*)(_this + 0xA3)) = 0;
	...
	comp_data->promotes_to = BEL_THIRD_VV_9CF();
	BYTE selected = get_country(NATION_BELGIUM_9CF())->NationLeagueSelected;
	if ((selected & 4) == 0) {
		comp_data->relegates_to = -1;
	}
	else {
		comp_data->relegates_to = BEL_FOURTH_VV_9CF();
	}
...
void __declspec(naked) bel_third_ac_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bel_third_ac_subs
		add esp, 0x4
		ret
	}
}

void block_reserve_promotion_bel_third_ac(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
char bel_third_ac_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
	...
	DWORD v1 = *(DWORD*)_this;
	block_reserve_promotion_bel_third_ac(_this);
	(*(int(__thiscall**)(BYTE*))(v1 + 0x5C))(_this);
	sub_68AA80(_this);
	return sub_79CEE0((BYTE*)*b74340, (BYTE*)(data->competition_db));
}

void __declspec(naked) bel_third_ac_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call bel_third_ac_update
		add esp, 0x4
		ret
	}
}

void bel_third_ac_split_under(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
char bel_third_ac_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
		if (comp_data->n_rounds == 2) {
			...
			if (is_finished) {
				bel_third_ac_split_under(_this);
			}
		}
	}
...
void __declspec(naked) bel_third_ac_table_split_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call bel_third_ac_table_split
		add esp, 0xc
		ret 8
	}
}

void bel_third_ac_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(bel_third_ac_vtable->vtable_ptr);
	bel_third_ac_vtable->SetPointer(VTableInitFree, (DWORD)&bel_third_ac_free_c);
	bel_third_ac_vtable->SetPointer(VTableSubsRounds, (DWORD)&bel_third_ac_subs_c);
	...
	if (configFile.GetBool("showThirdPlaceInHistory", true)) bel_third_ac_vtable->SetPointer(VTable21, 0x4110b0);
	data->year = year;
