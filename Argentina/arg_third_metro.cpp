...
int arg_third_metro_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) arg_third_metro_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call arg_third_metro_table_fates
		add esp, 0x1c
		ret 0x18
	}
}
...
void arg_third_metro_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	...
	arg_third_metro_vtable->SetPointer(VTableTableFates, (DWORD)&arg_third_metro_table_fates_c);
