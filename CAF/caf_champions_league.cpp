...
void __declspec(naked) caf_champions_league_free_c()
{
	__asm
	{
		mov eax, esp
...
DWORD caf_champions_league_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, QuarterFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, AwayGoalsPenaltiesNoExtraTime_2, 8, 8, 4, 8, 0, 0, 2, 7, 0, 0, 669145);
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, AwayGoalsPenaltiesNoExtraTime_2, 8, 4, 2, 0, 0, 0, 2, 7, 0, 0, 892193);
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 0, NoTiebreak_1, AwayGoalsPenaltiesNoExtraTime_2, 8, 2, 1, 0, 0, 0, 2, 9, 0, 4460964, 1486988);
...
void __declspec(naked) caf_champions_league_fixture_caller()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_champions_league_set_champion_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_champions_league_reputation_setup_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_champions_league_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_champions_league_update_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_champions_league_stages_create_c()
{
	__asm
	{
		mov eax, esp
...
int caf_cl_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) caf_cl_stage_news_c()
{
	__asm
	{
		mov eax, esp
