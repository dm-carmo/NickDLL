...
void __declspec(naked) caf_confederation_cup_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_confederation_cup_fixture_caller()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_confederation_cup_set_champion_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void caf_confed_team_selection() {
	...
	for (cm3_nations* caf_nation : caf_nations) {
		...
		else if (excluded_count < 2 && find(caf_worst.begin(), caf_worst.end(), caf_nation->NationName) != caf_worst.end()) {
			excluded_count++;
			continue;
		}
		BYTE j = 0;
...
void __declspec(naked) caf_confederation_cup_reputation_setup_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_confederation_cup_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_confederation_cup_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
	...
	for (char al = 0; al < 4; al++) {
		...
		for (int i = 0; i < total_teams; i++) {
			...
			if (tls.league_fate != Qualified1) {
				...
				sub_48E3E0(_this, (BYTE*)tls.club, 180000, 0, -1, GroupStage, 0, -2);
			}
		}
	}
}
...
void __declspec(naked) caf_confederation_cup_stages_create_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) caf_confederation_cup_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
