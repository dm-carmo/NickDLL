...
DWORD irl_cup_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		AddPlayoffFixture(pMem, fixture_id, Date(year, 9, 5), year, Friday, Evening);
		...
		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 9, 6), year, Saturday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 10, 10), year, Friday, Evening);
		...
		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 10, 11), year, Saturday);
...
void irl_cup_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_518640(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = irl_cup_vtable;
	data->year = year;
	data->f171 = 0;
	data->f68 = -1;
...
void __declspec(naked) irl_cup_init_c()
{
	__asm
	{
		mov eax, esp
...
char irl_cup_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	if (data->teams_list) {
...
void __declspec(naked) irl_cup_update_c()
{
	__asm
	{
		mov eax, esp
...
void setup_irl_cup() {
	...
	WriteVTablePtr(irl_cup_vtable, VTableEoSUpdate, (DWORD)&irl_cup_update_c);
	WriteVTablePtr(irl_cup_vtable, VTableSubsRounds, 0x858e70);
}