...
int init_leagues_list() {
	...
	pnd_list[idx].setup_function_addr = (DWORD)&world_cup_comps_setup_c;
	...
	convert_to_cm_date(pnd_list[idx].end_date, 18, December, *current_year + 1, -1);
	convert_to_cm_date(cm_date, 1, January, START_YEAR, -1);
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(cm_date, 20, June, START_YEAR, -1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	pnd_list[idx].main_cup = get_comp(MAR_THRONE_CUP_9CF());
	idx++;

	cm_date = new BYTE[8];
	pnd_list[idx].nation = get_country(NATION_UKRAINE_9CF());
	pnd_list[idx].setup_function_addr = (DWORD)&ukr_setup_c;
	convert_to_cm_date(pnd_list[idx].start_date, 25, July, *current_year, -1);
	convert_to_cm_date(pnd_list[idx].end_date, 15, June, *current_year, -1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, START_YEAR, -1);
...
void setup_leagues_setup() {
	...
	WriteBytes(0x6b6912, 1, 6);
	WriteBytes(0x6b6916, 1, 8);
	WriteBytes(0x6b6917, 1, 0x7f);
	...
	// BIG FIX: expanded so that both OFC and CAF can show the games as usual
	WriteBytes(0x5acbe2, 1, 0x28);
	WriteBytes(0x5acbe9, 1, 0x2c);
	WriteBytes(0x5acbf0, 1, 0x30);
	WriteBytes(0x5acbfb, 1, 0x4c);
	WriteBytes(0x5acc02, 1, 0x34);
	WriteBytes(0x5acc0f, 1, 0x2c);
	WriteBytes(0x5acc13, 1, 0x24);
	WriteBytes(0x5acc17, 1, 0x30);
	WriteBytes(0x5acc1b, 1, 0x28);
	WriteBytes(0x5acc77, 10, 0xC7, 0x44, 0x24, 0x20, 0x06, 0x07, 0x08, 0xFF, 0x90, 0x90);
	WriteBytes(0x5acc8f, 1, 0x23);
	WriteBytes(0x5acc9e, 1, 0x20);
	WriteBytes(0x5acce3, 1, 0x54);
	WriteBytes(0x5acd1c, 1, 0x5C);
	WriteBytes(0x5acd23, 1, 0x50);
	WriteBytes(0x5acd2a, 1, 0x2C);
	WriteBytes(0x5acd39, 1, 0x24);
	WriteBytes(0x5acd3e, 1, 0x48);
	WriteBytes(0x5acd51, 1, 0x44);
	WriteBytes(0x5acd79, 1, 0x34);
	WriteBytes(0x5acd7e, 1, 0x34);
	WriteBytes(0x5acda1, 1, 0x58);
	WriteBytes(0x5acdd8, 1, 0x54);
	WriteBytes(0x5acdfa, 1, 0x28);
	WriteBytes(0x5ace0d, 1, 0x24);
	WriteBytes(0x5ace11, 1, 0x2C);
	WriteBytes(0x5ace2a, 1, 0x30);
	WriteBytes(0x5ace32, 1, 0x30);
	WriteBytes(0x5ace3d, 1, 0x30);
	WriteBytes(0x5ace47, 1, 0x2C);
	WriteBytes(0x5ace51, 1, 0x24);
	WriteBytes(0x5ace55, 1, 0x2C);
	WriteBytes(0x5ace5e, 1, 0x24);
	WriteBytes(0x5ace67, 1, 0x38);
	WriteBytes(0x5ace6b, 1, 0x38);
	WriteBytes(0x5ace85, 1, 0x38);
	WriteBytes(0x5ace89, 1, 0x38);
	WriteBytes(0x5acecb, 1, 0x28);
	WriteBytes(0x5aced2, 1, 0x28);
