...
void __declspec(naked) cro_third_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) cro_third_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) cro_third_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) cro_third_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) cro_third_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int DrugaNLTableIndicators(BYTE* _this, DWORD* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage == 0) {
		...
		case Promoted:
			...
			return 0;
		case TopPlayoff:
			staff_history_qualified_868DD0(staff_hist_ptr, club, (DWORD)(cro_second), None, Playoff, 0x1E);
			return 0;
		case BottomPlayoff:
...
void __declspec(naked) cro_third_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
