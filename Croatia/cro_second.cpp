...
void cro_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = cro_second_vtable;
	sub_687970(_this, 0);
...
void cro_second_free(BYTE* _this, BYTE a2) {
	cro_second_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) cro_second_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call cro_second_free
		add esp, 0x8
		ret 4
	}
}

void cro_second_subs(BYTE* _this)
{
...
void __declspec(naked) cro_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call cro_second_subs
		add esp, 0x4
		ret
	}
}

DWORD cro_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) cro_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call cro_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

char cro_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
void __declspec(naked) cro_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call cro_second_update
		add esp, 0x4
		ret
	}
}

void cro_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
...
void cro_second_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (ret != 0) {
			(*(void(__thiscall**)(BYTE*))(v1 + 0x94))(cro_third);
			current++;
			comp_data->current_stage = current;
			if (current == 0) {
				cro_second_playoff_under(_this);
			}
		}
	}
}

void __declspec(naked) cro_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call cro_second_playoffs_c
		add esp, 0x4
		ret
	}
}

int cro_second_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		cm3_club_comps* cro_third = get_comp(CRO_THIRD_9CF());
		if (club->ClubDivision == cro_third) {
			comp_stats* cro_third_data = (comp_stats*)get_loaded_league(CRO_THIRD_9CF());
			...
			for (int i = 0; i < num_teams; i++) {
				...
				case TopPlayoff:
					...
					*a5 = 1;
					return 0;
				case Promoted:
					staff_history_qualified_86BDD0(staff_hist_ptr, club, (DWORD)cro_third, *(WORD*)(round_data + 0x32),
...
void __declspec(naked) cro_second_table_fates_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) cro_second_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
...
int cro_second_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) cro_second_stage_news_c()
{
	__asm
	{
		mov eax, esp
...
void setup_cro_second()
{
	...
	WriteVTablePtr(cro_second_vtable, VTableTableFates, (DWORD)&cro_second_table_fates_c);
	WriteVTablePtr(cro_second_vtable, VTableStageNews, (DWORD)&cro_second_stage_news_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) WriteVTablePtr(cro_second_vtable, VTableShowThirdInHistory, 0x4110b0);
}
