...
DWORD CreatePolandSuperFixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 0;
...
void __declspec(naked) pol_super_fixture_caller()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call CreatePolandSuperFixtures
		add esp, 0x14
		ret 0x10
	}
}
