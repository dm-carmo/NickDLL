...
void pol_cup_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_518640(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = pol_cup_vtable;
	data->year = year;
	data->f171 = 0;
	data->f68 = -1;
...
void __declspec(naked) pol_cup_init_c()
{
	__asm
	{
		mov eax, esp
...
char pol_cup_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	if (data->teams_list) {
...
void __declspec(naked) pol_cup_update_c()
{
	__asm
	{
		mov eax, esp
...
void setup_pol_cup()
{
	...
	WriteVTablePtr(pol_cup_vtable, VTableEoSUpdate, (DWORD)&pol_cup_update_c);
	WriteVTablePtr(pol_cup_vtable, VTableSubsRounds, 0x858e70);
}
