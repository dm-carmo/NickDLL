...
void eng_conf_s_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	...
	if (configFile.GetBool("showThirdPlaceInHistory", true)) eng_conf_s_vtable->SetPointer(VTable21, 0x4110b0);
	if (configFile.GetBool("showPlayoffWinnerInHistory", true)) eng_conf_s_vtable->SetPointer(VTable35, 0x404480);
	data->year = year;
