...
DWORD eng_league_2_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 0) {
		...
		*stage_name_id = Playoff;
		...
		AddPlayoffFixture(pMem, fixture_id, Date(year + 1, 5, 25), year, Monday, Afternoon, NationalStadium);
...
void setup_eng_league_2() {
	...
	if (configFile.GetBool("showThirdPlaceInHistory", true)) WriteVTablePtr(eng_league_2_vtable, VTable21, 0x4110b0);
	if (configFile.GetBool("showPlayoffWinnerInHistory", true)) WriteVTablePtr(eng_league_2_vtable, VTable35, 0x404480);
}