...
DWORD eng_champ_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 0) {
		...
		*stage_name_id = Playoff;
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup3 | NoAwayGoals, Penalties | ExtraTime | NoAwayGoals, 5, 4, 2, 2, 4, 0, 2, 4);
...
void setup_eng_champ() {
	...
	if (configFile.GetBool("showThirdPlaceInHistory", true)) WriteVTablePtr(eng_champ_vtable, VTableShowThirdInHistory, 0x4110b0);
	if (configFile.GetBool("showPlayoffWinnerInHistory", true)) WriteVTablePtr(eng_champ_vtable, VTableShowHostsInHistory, 0x404480);
}