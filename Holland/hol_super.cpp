...
DWORD hol_super_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, None, 1, Penalties, NoTiebreak, 6, 2, 1, 2, 0, 0, 1, 0);
...
int hol_super_teams(BYTE* _this) {
	...
	cm3_club_comps* league = get_comp(HOL_FIRST_9CF());
	cm3_club_comps* cup = get_comp(HOL_CUP_9CF());
...
char hol_super_update(BYTE* _this) {
	...
	data->f171 = 0;
	*((BYTE*)(_this + 0xB1)) = 0;
	hol_super_teams(_this);
	DWORD v1 = *(DWORD*)_this;
	(*(int(__thiscall**)(BYTE*))(v1 + 0x8C))(_this);
	return (*(int(__thiscall**)(BYTE*))(v1 + 0x94))(_this);
}

void __declspec(naked) hol_super_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call hol_super_update
		add esp, 0x4
		ret
	}
}

void hol_super_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_518640(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = hol_super_vtable;
	data->year = year;
	data->f171 = 0;
	data->f68 = -1;
...
void setup_hol_super()
{
	...
	WriteVTablePtr(hol_super_vtable, VTableEoSUpdate, (DWORD)&hol_super_update_c);
	WriteVTablePtr(hol_super_vtable, VTableSubsRounds, 0x858e70);
}
