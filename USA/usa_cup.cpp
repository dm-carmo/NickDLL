...
DWORD usa_cup_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 0, Penalties | ExtraTime, NoTiebreak, 6, 2, 1, 0, 0, 0, 1, 0, 0, prizeMoneyFile.GetInt("usa_cup_final_win"), prizeMoneyFile.GetInt("usa_cup_final_lose"));
...
void __declspec(naked) usa_cup_fixture_caller()
{
	__asm
	{
		mov eax, esp
...
void usa_cup_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_518640(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = usa_cup_vtable;
	data->year = year;
	data->f171 = 0;
	data->f68 = -1;
...
void __declspec(naked) usa_cup_init_c()
{
	__asm
	{
		mov eax, esp
...
char usa_cup_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	if (data->teams_list) {
...
void __declspec(naked) usa_cup_update_c()
{
	__asm
	{
		mov eax, esp
...
void setup_usa_cup()
{
	...
	WriteVTablePtr(usa_cup_vtable, VTableEoSUpdate, (DWORD)&usa_cup_update_c);
	WriteVTablePtr(usa_cup_vtable, VTableSubsRounds, 0x858e70);
}
