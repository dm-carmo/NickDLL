...
DWORD CreateFranceCupFixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 0;
...
void __declspec(naked) fra_cup_fixture_caller()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int fra_cup_teams(BYTE* _this) {
	...
	// Lower
	vector<cm3_clubs*> division_clubs = find_clubs_of_comp(FRA_CFA_9CF());
	...
	// National
	division_clubs = find_clubs_of_comp(FRA_NATIONAL_9CF());
