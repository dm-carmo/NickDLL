...
vtable* srb_second_vtable = new vtable((BYTE*)0x969798, 0xB4);

void srb_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(srb_second_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void srb_second_free(BYTE* _this, BYTE a2) {
	srb_second_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) srb_second_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call srb_second_free
		add esp, 0x8
		ret 4
	}
}

int srb_second_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 8;
...
DWORD srb_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = None;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 8, 19), year, Wednesday, Evening);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 9, 16), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 9, 26), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 10, 3), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 11, 14), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 12, 12), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year + 1, 3, 27), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		AddFixtureNoTV(pMem, fixture_id++, Date(year + 1, 4, 7), year, Sunday);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year + 1, 4, 21), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		*stage_name_id = ChampionshipGroup + stage_idx;
		...
		AddFixtureNoTV(pMem, fixture_id++, Date(year + 1, 4, 21), year, Wednesday, Evening);
...
void __declspec(naked) srb_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call srb_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void srb_second_subs(BYTE* _this)
{
	...
	*((DWORD*)(_this + 0xA3)) = 0;
	...
	if (comp_data->year == 2026) comp_data->relegations = 6;
	else comp_data->relegations = 4;

	comp_data->promotes_to = SRB_FIRST_9CF();
...
void __declspec(naked) srb_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call srb_second_subs
		add esp, 0x4
		ret
	}
}

char srb_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
	...
	srb_second_subs(_this);
	AddTeams(_this);
	SetupTVMoney(_this, prizeMoneyFile.GetInt("srb_second_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) srb_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call srb_second_update
		add esp, 0x4
		ret
	}
}

void srb_second_split_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	...
	data->n_rounds = 3;
	*((DWORD*)(_this + 0xA3)) = (DWORD)&srb_second_7F3220;
}

char srb_second_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) srb_second_table_split_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call srb_second_table_split
		add esp, 0xc
		ret 8
	}
}

void srb_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(srb_second_vtable->vtable_ptr);
	srb_second_vtable->SetPointer(VTableInitFree, (DWORD)&srb_second_free_c);
	srb_second_vtable->SetPointer(VTableSubsRounds, (DWORD)&srb_second_subs_c);
	srb_second_vtable->SetPointer(VTableEoSUpdate, (DWORD)&srb_second_update_c);
	srb_second_vtable->SetPointer(VTableFixtures, (DWORD)&srb_second_fixtures_c);
	srb_second_vtable->SetPointer(VTableLeagueSplit, (DWORD)&srb_second_table_split_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) srb_second_vtable->SetPointer(VTableShowThirdInHistory, 0x4110b0);
	data->year = year;
