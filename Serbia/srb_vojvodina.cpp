...
vtable* srb_vojvodina_vtable = new vtable((BYTE*)0x96CAF4, 0xB4);

void srb_vojvodina_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(srb_vojvodina_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void srb_vojvodina_free(BYTE* _this, BYTE a2) {
	srb_vojvodina_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) srb_vojvodina_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call srb_vojvodina_free
		add esp, 0x8
		ret 4
	}
}

void srb_vojvodina_subs(BYTE* _this)
{
...
void __declspec(naked) srb_vojvodina_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call srb_vojvodina_subs
		add esp, 0x4
		ret
	}
}

DWORD srb_vojvodina_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) srb_vojvodina_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call srb_vojvodina_fixtures
		add esp, 0x14
		ret 0x10
	}
}

int srb_vojvodina_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) srb_vojvodina_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call srb_vojvodina_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

char srb_vojvodina_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
void __declspec(naked) srb_vojvodina_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call srb_vojvodina_update
		add esp, 0x4
		ret
	}
}

void srb_vojvodina_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(srb_vojvodina_vtable->vtable_ptr);
	srb_vojvodina_vtable->SetPointer(VTableEoSUpdate, (DWORD)&srb_vojvodina_update_c);
	srb_vojvodina_vtable->SetPointer(VTableSubsRounds, (DWORD)&srb_vojvodina_subs_c);
	srb_vojvodina_vtable->SetPointer(VTableFixtures, (DWORD)&srb_vojvodina_fixtures_c);
	srb_vojvodina_vtable->SetPointer(VTableInitFree, (DWORD)&srb_vojvodina_free_c);
	srb_vojvodina_vtable->SetPointer(VTableTableFates, (DWORD)&srb_vojvodina_table_fates_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) srb_vojvodina_vtable->SetPointer(VTableShowThirdInHistory, 0x4110b0);
	data->year = year;
