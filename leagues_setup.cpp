...
int replacement_667150() {
	...
	convert_to_cm_date(pnd_list[idx].start_date, 1, July, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 1, July, *current_year, (DWORD*)-1);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 22, June, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 20, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	...
	convert_to_cm_date(pnd_list[idx].start_date, 23, June, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 20, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 21, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].end_date, 1, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 1, February, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].end_date, 1, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 21, December, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].end_date, 1, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 21, February, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].end_date, 1, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 21, July, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 9, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 19, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 1, August, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	idx++;
	...
	convert_to_cm_date(pnd_list[idx].start_date, 2, October, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 10, June, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 1, August, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	idx++;
	...
	convert_to_cm_date(pnd_list[idx].start_date, 7, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 1, June, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	...
	if (configFile.GetBool("applyBrazil", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 30, January, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 17, December, *current_year + 1, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x832980;
		convert_to_cm_date(pnd_list[idx].start_date, 18, January, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 14, December, *current_year + 1, (DWORD*)-1);
	}
	...
	if (configFile.GetBool("applyCroatia", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].end_date, 6, June, *current_year, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x8330B0;
		convert_to_cm_date(pnd_list[idx].end_date, 28, May, *current_year, (DWORD*)-1);
	}
	convert_to_cm_date(pnd_list[idx].start_date, 24, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyDenmark", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].end_date, 31, May, *current_year, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x833400;
		convert_to_cm_date(pnd_list[idx].end_date, 14, June, *current_year, (DWORD*)-1);
	}
	convert_to_cm_date(pnd_list[idx].start_date, 27, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 10, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 31, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyFinland", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 5, April, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 1, January, 2025, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x833CF0;
		convert_to_cm_date(pnd_list[idx].start_date, 29, April, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 27, December, 2025, (DWORD*)-1);
	}
	convert_to_cm_date(pnd_list[idx].end_date, 12, November, *current_year + 1, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 9, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 24, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	else
	{
		...
		convert_to_cm_date(pnd_list[idx].end_date, 26, May, *current_year, (DWORD*)-1);
	}
	convert_to_cm_date(pnd_list[idx].start_date, 29, July, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyGreece", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 23, August, *current_year, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 28, May, *current_year, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x8347C0;
		convert_to_cm_date(pnd_list[idx].start_date, 15, August, *current_year, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 21, May, *current_year, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	else
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 16, August, *current_year, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 7, June, *current_year, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 1, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 18, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	else
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 25, August, *current_year, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 8, June, *current_year, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyJapan", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 14, February, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 9, December, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 1, January, 2025, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x835900;
		convert_to_cm_date(pnd_list[idx].start_date, 6, March, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 28, November, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 25, January, 2025, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 12, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 8, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyNorway", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 30, March, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 14, December, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 1, January, 2025, (DWORD*)-1);
	}
	else
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 12, April, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 31, October, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 27, November, 2025, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyPoland", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 20, July, *current_year, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x836350;
		convert_to_cm_date(pnd_list[idx].start_date, 17, July, *current_year, (DWORD*)-1);
	}
	convert_to_cm_date(pnd_list[idx].end_date, 5, June, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyPortugal", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 3, August, *current_year, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 10, June, *current_year, (DWORD*)-1);
	}
	else
	{
	...
	convert_to_cm_date(pnd_list[idx].start_date, 25, March, *current_year + 1, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 12, November, *current_year + 1, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 0;
	convert_to_cm_date(cm_date, 27, December, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 2, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 24, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applySouthKorea", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 15, February, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 8, December, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 1, January, 2025, (DWORD*)-1);
	}
	else
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 25, March, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 25, November, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 14, January, 2025, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 20, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 14, June, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applySweden", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 30, March, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 30, November, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 1, January, 2025, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x8379F0;
		convert_to_cm_date(pnd_list[idx].start_date, 6, April, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 26, October, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 27, December, 2025, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 8, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 20, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	if (configFile.GetBool("applyUSA", true))
	{
		...
		convert_to_cm_date(pnd_list[idx].start_date, 22, February, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 10, December, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 1, January, 2025, (DWORD*)-1);
	}
	else
	{
		pnd_list[idx].setup_function_addr = 0x8380C0;
		convert_to_cm_date(pnd_list[idx].start_date, 22, March, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(pnd_list[idx].end_date, 6, January, *current_year + 1, (DWORD*)-1);
		convert_to_cm_date(cm_date, 20, February, 2025, (DWORD*)-1);
	}
	pnd_list[idx].updates_in_june = 0;
	pnd_list[idx].update_day = *(WORD*)(cm_date);
	...
	convert_to_cm_date(pnd_list[idx].start_date, 19, August, *current_year, (DWORD*)-1);
	convert_to_cm_date(pnd_list[idx].end_date, 6, May, *current_year, (DWORD*)-1);
	pnd_list[idx].updates_in_june = 1;
	convert_to_cm_date(cm_date, 20, June, 2025, (DWORD*)-1);
	pnd_list[idx].update_day = *(WORD*)(cm_date);
