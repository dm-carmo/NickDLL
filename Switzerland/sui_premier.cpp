...
int sui_premier_table_indicators(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage == 0) {
		...
		if (club->ClubDivision == sui_first) {
			comp_stats* sui_first_data = (comp_stats*)get_loaded_league(SUI_FIRST_9CF());
...
int sui_premier_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) sui_premier_stage_news_c()
{
	__asm
	{
		mov eax, esp
