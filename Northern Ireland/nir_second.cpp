...
vtable* nir_second_vtable = new vtable((BYTE*)0x969798, 0xB4);
...
void __declspec(naked) nir_second_free_c()
{
	__asm
	{
		mov eax, esp
...
int nir_second_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 6;
...
void __declspec(naked) nir_second_subs_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) nir_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
...
char nir_second_update(BYTE* _this) {
	...
	AddTeams(_this);
	SetupTVMoney(_this, prizeMoneyFile.GetInt("nir_second_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) nir_second_update_c()
{
	__asm
	{
		mov eax, esp
...
void nir_second_split_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	...
	data->n_rounds = 4;
	*((DWORD*)(_this + 0xA3)) = (DWORD)&nir_second_7F3220;
}

char nir_second_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) nir_second_table_split_c()
{
	__asm
	{
		mov eax, esp
