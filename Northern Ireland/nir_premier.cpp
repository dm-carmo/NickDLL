...
DWORD* nir_premier_vtable = (DWORD*)0x96DCCC;

int nir_premier_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 6;
...
DWORD nir_premier_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) nir_premier_fixtures_c()
{
	__asm
	{
		...
		push ecx
		call nir_premier_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void nir_premier_subs(BYTE* _this)
{
	...
	*((DWORD*)(_this + 0xA3)) = 0;
	...
	comp_data->relegates_to = NIR_FIRST_9CF();
...
void __declspec(naked) nir_premier_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nir_premier_subs
		add esp, 0x4
		ret
	}
}

char nir_premier_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	...
	generic_prom_rel(NATION_NORTHERN_IRELAND_9CF(), A_LOWER_9CF(), NIR_SECOND_9CF(), 0);
	...
	nir_premier_subs(_this);
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("nir_prm_prize_money"));
	data->f225 = 1;
	SetupTVMoney(_this, prizeMoneyFile.GetInt("nir_prm_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	...
	v1 = *(DWORD*)nir_second;
	(*(int(__thiscall**)(BYTE*))(v1 + 0x8))(nir_second);
...
void __declspec(naked) nir_premier_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nir_premier_update
		add esp, 0x4
		ret
	}
}

void nir_premier_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = nir_premier_vtable;
	data->year = year;
...
char nir_premier_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) nir_premier_table_split_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nir_premier_table_split
		add esp, 0xc
		ret 8
	}
}

void nir_premier_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	comp_stats* nir_first_data = (comp_stats*)get_loaded_league(NIR_FIRST_9CF());
...
void nir_premier_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	long current = comp_data->current_stage;
	long max = comp_data->num_stages;
	if (current < max - 1) {
		BYTE* nir_first = get_loaded_league(NIR_FIRST_9CF());
		DWORD v1 = *(DWORD*)nir_first;
		char ret = (*(int(__thiscall**)(BYTE*, int, int))(v1 + 0x10))(nir_first, 0, 1);
		if (ret != 0) {
			(*(void(__thiscall**)(BYTE*))(v1 + 0x94))(nir_first);
			current++;
			if (current == 0) {
				comp_data->current_stage = current;
				nir_premier_playoff_under(_this);
			}
		}
	}
}
...
void __declspec(naked) nir_premier_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nir_premier_playoffs_c
		add esp, 0x4
		ret
	}
}

int nir_premier_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		cm3_club_comps* nir_first = get_comp(NIR_FIRST_9CF());
		BYTE* rounds = ((comp_stats*)(comp_data->stages[stage]))->rounds_list;
		if (club->ClubDivision == nir_first) {
			comp_stats* nir_first_data = (comp_stats*)get_loaded_league(NIR_FIRST_9CF());
			WORD num_teams = nir_first_data->n_teams;
			if (num_teams <= 0) return 0;
			team_league_stats* table = (team_league_stats*)(nir_first_data->team_league_table);
			WORD current_round = *(WORD*)(round_data + 0x34);
			for (int i = 0; i < num_teams; i++) {
				if (table[i].club != club) continue;
				switch (fate) {
				case TopPlayoff:
					staff_history_promoted_869480(staff_hist_ptr, club, (DWORD)nir_first, 0x32);
...
void __declspec(naked) nir_premier_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nir_premier_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

void nir_premier_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	char ret_max = max;
	if (stage == 0) {
		comp_stats* d2_comp_data = (comp_stats*)get_loaded_league(NIR_FIRST_9CF());
		cm3_clubs* club_data = (cm3_clubs*)club;
		if (club_data->ClubDivision->ClubCompID == NIR_FIRST_9CF()) {
...
void __declspec(naked) nir_premier_reputation_calc_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nir_premier_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int nir_premier_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) nir_premier_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nir_premier_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void setup_nir_premier()
{
	WriteVTablePtr(nir_premier_vtable, VTableSubsRounds, (DWORD)&nir_premier_subs_c);
	...
	WriteVTablePtr(nir_premier_vtable, VTableTableFates, (DWORD)&nir_premier_table_fates_c);
	WriteVTablePtr(nir_premier_vtable, VTableStageNews, (DWORD)&nir_premier_stage_news_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) WriteVTablePtr(nir_premier_vtable, VTableShowThirdInHistory, 0x4110b0);
}