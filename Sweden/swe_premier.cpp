...
char swe_premier_update(BYTE* _this) {
	...
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("swe_prm_prize_money"));
	data->f225 = 1;
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	DWORD v1 = *(DWORD*)_this;
...
int swe_premier_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) swe_premier_table_fates_c()
{
	__asm
	{
		mov eax, esp
...
int swe_premier_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) swe_premier_stage_news_c()
{
	__asm
	{
		mov eax, esp
...
void setup_swe_premier()
{
	...
	WriteVTablePtr(swe_premier_vtable, VTableTableFates, (DWORD)&swe_premier_table_fates_c);
	WriteVTablePtr(swe_premier_vtable, VTablePromRelUpdate, (DWORD)&swe_premier_prom_rel_update_c);
	WriteVTablePtr(swe_premier_vtable, VTableStageNews, (DWORD)&swe_premier_stage_news_c);
