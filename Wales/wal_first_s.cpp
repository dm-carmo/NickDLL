...
vtable* wal_first_s_vtable = new vtable((BYTE*)0x969A74, 0xB4);

void wal_first_s_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(wal_first_s_vtable->vtable_ptr);
	DWORD x = 0;
	sub_687970(_this, 0);
...
void wal_first_s_free(BYTE* _this, BYTE a2) {
	wal_first_s_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) wal_first_s_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call wal_first_s_free
		add esp, 0x8
		ret 4
	}
}

int wal_first_s_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = WAL_FIRST_NORTH_9CF();
...
DWORD wal_first_s_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) wal_first_s_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call wal_first_s_fixtures
		add esp, 0x14
		ret 0x10
	}
}

char wal_first_s_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
...
void __declspec(naked) wal_first_s_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call wal_first_s_update
		add esp, 0x4
		ret
	}
}

void __declspec(naked) wal_first_s_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call wal_first_s_subs
		add esp, 0x4
		ret
	}
}

void wal_first_s_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(wal_first_s_vtable->vtable_ptr);
	wal_first_s_vtable->SetPointer(VTableInitFree, (DWORD)&wal_first_s_free_c);
