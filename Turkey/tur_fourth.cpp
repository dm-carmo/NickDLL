...
vtable* tur_fourth_vtable = new vtable((BYTE*)0x970694, 0xB4);
...
void __declspec(naked) tur_fourth_set_champion_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call tur_fourth_set_champion
		add esp, 0x4
		ret 0
	}
}
...
void __declspec(naked) tur_fourth_last_positions_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call tur_fourth_last_positions
		add esp, 0x4
		ret
	}
}

void tur_fourth_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(tur_fourth_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void tur_fourth_free(BYTE* _this, BYTE a2) {
	tur_fourth_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) tur_fourth_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call tur_fourth_free
		add esp, 0x8
		ret 4
	}
}

void tur_fourth_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = TUR_THIRD_9CF();
...
void __declspec(naked) tur_fourth_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call tur_fourth_subs
		add esp, 0x4
		ret
	}
}
...
void __declspec(naked) tur_fourth_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call tur_fourth_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void tur_fourth_setup_groups(BYTE* _this, BYTE idx) {
	DWORD v1 = *(DWORD*)_this;
	WORD num_rounds = 0;
...
void tur_fourth_reputation_setup(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) tur_fourth_reputation_setup_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call tur_fourth_reputation_setup
		add esp, 0x4
		ret
	}
}

char tur_fourth_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
void __declspec(naked) tur_fourth_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call tur_fourth_update
		add esp, 0x4
		ret
	}
}
...
void tur_fourth_playoffs_create(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) tur_fourth_playoffs_create_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call tur_fourth_playoffs_create
		add esp, 0x4
		ret
	}
}
...
void tur_fourth_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) tur_fourth_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call tur_fourth_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int tur_fourth_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) tur_fourth_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call tur_fourth_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void tur_fourth_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(tur_fourth_vtable->vtable_ptr);
	tur_fourth_vtable->SetPointer(VTableInitFree, (DWORD)&tur_fourth_free_c);
	tur_fourth_vtable->SetPointer(VTableEoSUpdate, (DWORD)&tur_fourth_update_c);
	tur_fourth_vtable->SetPointer(VTableFixtures, (DWORD)&tur_fourth_fixtures_c);
	...
	tur_fourth_vtable->SetPointer(VTableStageNews, (DWORD)&tur_fourth_stage_news_c);
	data->year = year;
