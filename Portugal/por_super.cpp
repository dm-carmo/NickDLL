...
DWORD por_super_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		AddPlayoffFixture(pMem, fixture_id, Date(year, 8, 3), year, Sunday, Afternoon, NeutralStadium);
		FillFixtureDetails(pMem, fixture_id++, None, 8, Penalties | ExtraTime, NoTiebreak, 6, 2, 1, 2, 0, 0, 1, 0);
...
int por_super_teams(BYTE* _this) {
	...
	cm3_club_comps* league = get_comp(POR_FIRST_9CF());
	cm3_club_comps* cup = get_comp(POR_CUP_9CF());
...
char por_super_update(BYTE* _this) {
	...
	data->f171 = 0;
	*((BYTE*)(_this + 0xB1)) = 0;
	por_super_teams(_this);
	DWORD v1 = *(DWORD*)_this;
	(*(int(__thiscall**)(BYTE*))(v1 + 0x8C))(_this);
	return (*(int(__thiscall**)(BYTE*))(v1 + 0x94))(_this);
}

void __declspec(naked) por_super_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call por_super_update
		add esp, 0x4
		ret
	}
}

void por_super_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_518640(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = por_super_vtable;
	data->year = year;
	data->f171 = 0;
	data->f68 = -1;
...
void setup_por_super()
{
	...
	WriteVTablePtr(por_super_vtable, VTableEoSUpdate, (DWORD)&por_super_update_c);
	WriteVTablePtr(por_super_vtable, VTableSubsRounds, 0x858e70);
}
