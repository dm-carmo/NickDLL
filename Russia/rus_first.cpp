...
DWORD* rus_first_vtable = (DWORD*)0x96F1C4;

void rus_first_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = RUS_PREMIER_9CF();
	comp_data->relegates_to = RUS_SECOND_A_9CF();
...
void __declspec(naked) rus_first_subs_c()
{
	__asm
	{
		mov eax, esp
...
DWORD rus_first_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) rus_first_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call rus_first_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void rus_first_block_promotion(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
...
char rus_first_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
	...
	rus_first_subs(_this);
	AddTeams(_this);
	rus_first_block_promotion(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("rus_first_prize_money"));
	data->f225 = 1;
	sub_6835C0(_this);
	BYTE* edx = 0;
...
void __declspec(naked) rus_first_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call rus_first_update
		add esp, 0x4
		ret
	}
}

void rus_first_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = rus_first_vtable;
	data->year = year;
...
int rus_first_table_indicators(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if(stage == -1) {
		...
		case Promoted:
			...
			return 0;
		case TopPlayoff:
			staff_history_qualified_86BDD0(staff_hist_ptr, club, (DWORD)(rus_premier), None, Playoff, 0x1E);
			return 0;
		case BottomPlayoff:
