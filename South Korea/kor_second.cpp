...
void __declspec(naked) kor_second_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_second_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
DWORD kor_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 10, 8), year, Wednesday, Evening);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 10, 19), year, Saturday);
		tv_id = 0;
...
void __declspec(naked) kor_second_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_second_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_second_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_second_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
char kor_second_update(BYTE* _this) {
	...
	AddTeams(_this);
	add_tv_money_683010(_this, 6750, 0);
	sub_6835C0(_this);
	BYTE* edx = 0;
...
void __declspec(naked) kor_second_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
