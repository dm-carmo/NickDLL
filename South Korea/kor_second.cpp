...
void kor_second_free(BYTE* _this, BYTE a2) {
	kor_second_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) kor_second_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call kor_second_free
		add esp, 0x8
		ret 4
	}
}

void kor_second_subs(BYTE* _this)
{
	comp_stats* comp_data = (comp_stats*)_this;
	...
	comp_data->promotes_to = KOR_FIRST_9CF();
...
void __declspec(naked) kor_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call kor_second_subs
		add esp, 0x4
		ret
	}
}

DWORD kor_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = PromotionPlayoff;
		...
		AddPlayoffFixture(pMem, fixture_id, Date(year, 12, 4), year, Thursday, Evening);
...
void __declspec(naked) kor_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call kor_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void kor_second_playoffs_prom(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
...
void kor_second_playoffs_rele(BYTE* _this) {
	char stage_num = 1;
	...
	vector<cm3_clubs*> available_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_SOUTH_KOREA_9CF());
	vector<cm3_clubs*> random_lower = get_random_weighted_clubs(available_clubs, 1, true);
	*((DWORD*)(&pTeams[1])) = (DWORD)random_lower[0];
...
void kor_second_playoffs_create_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0)
		{
			...
			kor_second_playoffs_prom(_this);
			kor_second_playoffs_rele(_this);
		}
	}
}

void __declspec(naked) kor_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call kor_second_playoffs_create_c
		add esp, 0x4
		ret
	}
}
...
void kor_second_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) kor_second_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call kor_second_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

char kor_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
void __declspec(naked) kor_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call kor_second_update
		add esp, 0x4
		ret
	}
}

int kor_second_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
	...
	if (stage_id == -1) return sub_48C6D0(_this, club_idx, fate, stage_id, stage_name_idx, round_data, a7, 0, a9, show_body_text, ret_str_ptr);
		...
		if (fate == TopPlayoff && !show_body_text) {
			cm3_club_comps* upper_comp = get_comp(KOR_FIRST_9CF());
...
void __declspec(naked) kor_second_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call kor_second_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void kor_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(kor_second_vtable->vtable_ptr);
	kor_second_vtable->SetPointer(VTableEoSUpdate, (DWORD)&kor_second_update_c);
	kor_second_vtable->SetPointer(VTableSubsRounds, (DWORD)&kor_second_subs_c);
