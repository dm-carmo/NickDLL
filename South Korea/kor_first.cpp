...
DWORD kor_first_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		int fixture_id = 0;
		int tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 2, 15), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 2, 22), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 3, 29), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 5, 6), year, Tuesday, Evening);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 5, 28), year, Wednesday, Evening);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 6, 17), year, Tuesday, Evening);
...
void __declspec(naked) kor_first_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_first_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_first_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void kor_restruct_2025() {
	cm3_club_comps* kor_first = &(*club_comps)[KOR_FIRST_9CF()];
	cm3_club_comps* kor_second = &(*club_comps)[KOR_SECOND_9CF()];
...
void __declspec(naked) kor_first_table_split_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		if (tls.league_fate == TopPlayoff) {
			*((DWORD*)(&pTeams[2 * j + 1])) = (DWORD)tls.club;
			if (j == 0)
			{
				...
				staff_history_qualified_86BDD0(staff_hist_ptr, (DWORD*)tls.club, (DWORD)comp_data->competition_db, None, Playoff, 0xF);
			}
			j++;
		}
	}
...
void __declspec(naked) kor_first_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int kor_first_table_indicators(BYTE* _this, DWORD* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		cm3_clubs* club_ptr = (cm3_clubs*)club;
		cm3_club_comps* kor_second = &(*club_comps)[KOR_SECOND_9CF()];
		if (club_ptr->ClubDivision == kor_second) {
			comp_stats* kor_second_data = (comp_stats*)get_loaded_league(KOR_SECOND_9CF());
...
void __declspec(naked) kor_first_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) kor_first_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void setup_kor_first()
{
	...
	WriteVTablePtr(kor_first_vtable, VTableStageNews, (DWORD)0x7f3080); // Scotland stage news contains champ/rel group news
