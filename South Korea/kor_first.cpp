...
DWORD* kor_first_vtable = (DWORD*)0x96CDD0;

int kor_first_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 6;
...
DWORD kor_first_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = None;
		...
		int fixture_id = 0;
		int tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 3, 1), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 3, 8), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 4, 22), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 5, 6), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 5, 13), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 7, 19), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 8, 2), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 8, 26), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 6, 9), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 11, 1), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 11, 8), year, Sunday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 12, 6), year, Sunday);
		...
		*stage_name_id = ChampionshipGroup + stage_idx - 1;
		...
		int fixture_id = 0;
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 11, 1), year, Sunday);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 11, 8), year, Sunday);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 11, 22), year, Sunday);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 11, 29), year, Sunday);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 12, 6), year, Sunday);
...
void __declspec(naked) kor_first_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call kor_first_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void kor_first_subs(BYTE* _this)
{
	...
	*((DWORD*)(_this + 0xA3)) = 0;
	...
	comp_data->relegates_to = KOR_SECOND_9CF();
...
void __declspec(naked) kor_first_subs_c()
{
	__asm
	{
		mov eax, esp
...
void __fastcall kor_second_relegation()
{
	cm3_clubs* relegate = 0;
	BYTE* comp_bytes = get_loaded_league(KOR_SECOND_9CF());
	...
	for (WORD i = 0; i < playoff_stage->n_teams; i++) {
		...
		if (t.f6 == 1) {
			if (t.club->ClubDivision != comp_data->competition_db) {
				promote = t.club;
				break;
			}
		}
...
char kor_first_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
...
void __declspec(naked) kor_first_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call kor_first_update
		add esp, 0x4
		ret
	}
}
...
char kor_first_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) kor_first_table_split_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call kor_first_table_split
		add esp, 0xc
		ret 8
	}
}

void kor_first_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	team_league_stats* table_teams = (team_league_stats*)(comp_data->team_league_table);
	int j = 0;
	for (int i = 0; i < total_teams && j < 2; i++) {
		team_league_stats tls = table_teams[i];
	...
	comp_stats* kor_second_data = (comp_stats*)get_loaded_league(KOR_SECOND_9CF());
	total_teams = kor_second_data->n_teams;
	table_teams = (team_league_stats*)(kor_second_data->team_league_table);
	j = 0;
	for (int i = 0; i < total_teams && j < 2; i++) {
		team_league_stats tls = table_teams[i];
		if (tls.league_fate == TopPlayoff) {
			*((DWORD*)(&pTeams[2 * j + 1])) = (DWORD)tls.club;
			if (j == 0)
			{
...
void kor_first_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	long current = comp_data->current_stage;
	long max = comp_data->num_stages;
	if (current < max - 1) {
		BYTE* kor_second = get_loaded_league(KOR_SECOND_9CF());
		comp_stats* kor_second_data = (comp_stats*)kor_second;
		BYTE* prom_playoff = (BYTE*)kor_second_data->stages[0];
		if (prom_playoff) {
...
void __declspec(naked) kor_first_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call kor_first_playoffs_c
		add esp, 0x4
		ret
	}
}
...
int kor_first_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage == 0) {
		...
		if (club->ClubDivision == kor_second) {
			comp_stats* kor_second_data = (comp_stats*)get_loaded_league(KOR_SECOND_9CF());
			WORD num_teams = kor_second_data->n_teams;
			if (num_teams <= 0) return 0;
			team_league_stats* table = (team_league_stats*)(kor_second_data->team_league_table);
			WORD current_round = *(WORD*)(round_data + 0x34);
			for (int i = 0; i < num_teams; i++) {
				if (table[i].club != club) continue;
				switch (fate) {
				case TopPlayoff:
					staff_history_promoted_869480(staff_hist_ptr, club, (DWORD)kor_second, 0x32);
...
void kor_first_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	char ret_max = max;
	if (stage == 0) {
		comp_stats* d2_comp_data = (comp_stats*)get_loaded_league(KOR_SECOND_9CF());
		cm3_clubs* club_data = (cm3_clubs*)club;
		if (club_data->ClubDivision->ClubCompID == KOR_SECOND_9CF()) {
...
void __declspec(naked) kor_first_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call kor_first_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int kor_first_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) kor_first_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call kor_first_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void setup_kor_first()
{
	WriteVTablePtr(kor_first_vtable, VTableSubsRounds, (DWORD)&kor_first_subs_c);
	WriteVTablePtr(kor_first_vtable, VTableEoSUpdate, (DWORD)&kor_first_update_c);
	WriteVTablePtr(kor_first_vtable, VTableFixtures, (DWORD)&kor_first_fixtures_c);
