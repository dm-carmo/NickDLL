...
char afc_champions_league_elite_update(BYTE* _this) {
	...
	data->f171 = 0;
	*((BYTE*)(_this + 0xB1)) = 0;
	afc_champions_league_elite_all_teams(_this); //584fa0
	afc_champions_league_elite_qualifier_teams(_this); //584990
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			*((DWORD*)(&pTeams[j])) = (DWORD)club;
			...
			sub_5999A0((BYTE*)ret, 360000);
			sub_48E3E0(_this, (BYTE*)club, 360000, 0, -1, GroupStage, 0, -2);
...
void afc_champions_league_elite_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	...
	*((DWORD*)(_this + 0xA3)) = (DWORD)(*(int(__thiscall**)(BYTE*, int, BYTE*, BYTE*, DWORD))(v1 + 0x3C))(_this, -1, _this + 0x3c, _this + 0x3a, 0);
	...
	unk1 = 0;
	data->f8 = (DWORD*)pMem2;
	afc_champions_league_elite_reputation_setup(_this); //586900
}
