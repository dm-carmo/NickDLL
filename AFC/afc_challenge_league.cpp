...
DWORD afc_challenge_league_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, QuarterFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, AwayGoalsPenaltiesNoExtraTime_2, 8, 8, 4, 8, 0, 0, 2, 7, 36000);
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, AwayGoalsPenaltiesNoExtraTime_2, 8, 4, 2, 0, 0, 0, 2, 7, 54000);
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 0, ExtraTimePenalties_1, NoTiebreak_2, 8, 2, 1, 0, 0, 0, 1, 0, 0, 450000, 225000);
	...
	for (int i = 0; i < 5; i++) {
		...
		for (int j = 0; j < group_teams; j++) {
			...
			*((DWORD*)(&pTeams[j])) = (DWORD)club;
			...
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club, 45000, 0, -1, GroupStage, 0, -2);
		}
