	...
	for (DWORD i = 0; i < *clubs_count; i++)
	{
		...
		if (club)
		{
			if (club->ClubDivision && club->ClubNation)
			{
				...
				if (nationID == NATION_SPAIN_9CF() && compID == SPA_FOURTH_9CF())
				{
...
void __fastcall spa_non_league_promotion(BYTE* _this)
{
	...
	for (DWORD i = 0; i < *clubs_count; i++)
	{
		...
		if (club)
		{
			if (club->ClubDivision && club->ClubNation)
			{
				DWORD compID = club->ClubDivision->ClubCompID;
				DWORD nationID = club->ClubNation->NationID;
				if (nationID == NATION_SPAIN_9CF() && compID == A_LOWER_9CF())
				{
					available_clubs.push_back(club);
				}
			}
	...
	sort(available_clubs.begin(), available_clubs.end(), compareClubRep);
	int max_to_check = (available_clubs.size() > 40 ? 40 : available_clubs.size());
	for (unsigned int i = 0; i < relegated_clubs.size(); i++)
	{
		int availableIdx = rand() % (max_to_check - i);
		cm3_clubs* clubToRelegate = relegated_clubs[i];
...
void __fastcall spa_qualify_teams_for_cup(BYTE* _this) {
	...
	// Lower
	vector<cm3_clubs*> division_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_SPAIN_9CF());
...
char spa_first_update(BYTE* _this) {
	...
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, 2374213);
	data->f225 = 1;
	SetupTVMoney(_this, 30217250, 0);
	sub_6835C0(_this);
	BYTE* edx = 0;
