...
void spa_second_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = SPA_FIRST_9CF();
	comp_data->relegates_to = SPA_THIRD_9CF();
...
void __declspec(naked) spa_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call spa_second_subs
		add esp, 0x4
		ret
	}
}

void block_reserve_promotion_spa_second(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
char spa_second_update(BYTE* _this) {
...
void __declspec(naked) spa_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call spa_second_update
		add esp, 0x4
		ret
	}
}

void spa_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = spa_second_vtable;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
		sub_9452CA_free(data->fixtures_table);
		data->fixtures_table = 0;
...
void spa_second_free(BYTE* _this, BYTE a2) {
	spa_second_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) spa_second_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call spa_second_free
		add esp, 0x8
		ret 4
	}
}

// adjusted calendar, ends 2 weeks before real date
DWORD spa_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) spa_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call spa_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void spa_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	...
	if (loaded) return;
	data->min_stadium_capacity = 6000;
	data->min_stadium_seats = 6000;
	data->f68 = -1;
	data->current_stage = -1;
	data->num_stages = 1;
...
void spa_second_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			spa_second_prom_playoffs(_this);
		}
	}
}

void __declspec(naked) spa_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call spa_second_playoffs_c
		add esp, 0x4
		ret
	}
}
...
void spa_second_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) spa_second_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call spa_second_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

void setup_spa_second()
{
	WriteVTablePtr(spa_second_vtable, VTableSubsRounds, (DWORD)&spa_second_subs_c);
	WriteVTablePtr(spa_second_vtable, VTableInitFree, (DWORD)&spa_second_free_c);
	WriteVTablePtr(spa_second_vtable, VTableEoSUpdate, (DWORD)&spa_second_update_c);
