...
DWORD den_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year + 1, 3, 25), year, Wednesday, Evening);
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year + 1, 5, 12), year, Wednesday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Tuesday, Evening);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year + 1, 5, 30), year, Saturday);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Friday, Evening);
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Sunday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		...
		*stage_name_id = ChampionshipGroup + stage_idx;
		...
		AddFixtureNoTV(pMem, fixture_id++, Date(year + 1, 5, 12), year, Wednesday, Evening);
...
void __declspec(naked) den_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_second_subs_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_second_update_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_second_table_split_c()
{
	__asm
	{
		mov eax, esp
