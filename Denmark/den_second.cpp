...
DWORD* den_second_vtable = (DWORD*)0x969798;

int den_second_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 6;
...
DWORD den_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) den_second_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call den_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void den_second_subs(BYTE* _this)
{
	...
	*((DWORD*)(_this + 0xA3)) = 0;
	...
	comp_data->promotes_to = DEN_FIRST_9CF();
	comp_data->relegates_to = DEN_THIRD_9CF();
...
void __declspec(naked) den_second_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call den_second_subs
		add esp, 0x4
		ret
	}
}

char den_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
...
void __declspec(naked) den_second_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call den_second_update
		add esp, 0x4
		ret
	}
}

void den_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = den_second_vtable;
	data->year = year;
	data->rules = 0x8;
	int loaded = sub_687B10(_this, 1);
	if (loaded) {
		if (data->n_rounds != 4) return;
		*((DWORD*)(_this + 0xA3)) = (DWORD)&den_second_7F3220;
		return;
	}
	comp->ClubCompReputation = 5;
	data->f68 = -1;
	data->current_stage = -1;
	data->num_stages = 0;
	den_second_subs(_this);
...
void den_second_split_under(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
char den_second_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) den_second_table_split_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call den_second_table_split
		add esp, 0xc
		ret 8
	}
}

void setup_den_second()
{
	WriteVTablePtr(den_second_vtable, VTableSubsRounds, (DWORD)&den_second_subs_c);
	...
	WriteVTablePtr(den_second_vtable, VTableStageNews, (DWORD)0x7f3080); // Scotland stage news contains champ/rel group news
}