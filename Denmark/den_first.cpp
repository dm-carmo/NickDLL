...
void __declspec(naked) den_first_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_first_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
char den_first_update(BYTE* _this) {
	...
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, 8739);
	data->f225 = 1;
	SetupTVMoney(_this, 69452, 0);
	sub_6835C0(_this);
	BYTE* edx = 0;
...
void __declspec(naked) den_first_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_first_table_split_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void setup_den_first()
{
	...
	WriteVTablePtr(den_first_vtable, VTableStageNews, (DWORD)0x7f3080); // Scotland stage news contains champ/rel group news
}