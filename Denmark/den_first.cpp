...
DWORD* den_first_vtable = (DWORD*)0x969630;

int den_first_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 6;
...
DWORD den_first_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) den_first_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call den_first_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void den_first_subs(BYTE* _this)
{
	...
	*((DWORD*)(_this + 0xA3)) = 0;
	...
	comp_data->promotes_to = DEN_PREMIER_9CF();
	comp_data->relegates_to = DEN_SECOND_9CF();
...
void __declspec(naked) den_first_subs_c()
{
	__asm
	{
		mov eax, esp
...
char den_first_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	...
	den_first_subs(_this);
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("den_first_prize_money"));
	data->f225 = 1;
	SetupTVMoney(_this, prizeMoneyFile.GetInt("den_first_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) den_first_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call den_first_update
		add esp, 0x4
		ret
	}
}

void den_first_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = den_first_vtable;
	data->year = year;
...
void den_first_split_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
...
char den_first_table_split(BYTE* _this, DWORD current_date, int a2) {
	if (a2) {
		comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) den_first_table_split_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call den_first_table_split
		add esp, 0xc
		ret 8
	}
}

void setup_den_first()
{
	WriteVTablePtr(den_first_vtable, VTableSubsRounds, (DWORD)&den_first_subs_c);
	WriteVTablePtr(den_first_vtable, VTableEoSUpdate, (DWORD)&den_first_update_c);
	WriteVTablePtr(den_first_vtable, VTableFixtures, (DWORD)&den_first_fixtures_c);
	WriteVTablePtr(den_first_vtable, VTableLeagueSplit, (DWORD)&den_first_table_split_c);
	WriteVTablePtr(den_first_vtable, VTableStageNews, 0x7f3080); // Scotland stage news contains champ/rel group news
	if (configFile.GetBool("showThirdPlaceInHistory", true)) WriteVTablePtr(den_first_vtable, VTableShowThirdInHistory, 0x4110b0);
}