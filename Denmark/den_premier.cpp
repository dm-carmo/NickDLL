...
void __declspec(naked) den_premier_fixtures_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_premier_subs_c()
{
	__asm
	{
		mov eax, esp
...
char den_premier_update(BYTE* _this) {
	...
	generic_prom_rel(NATION_DENMARK_9CF(), A_LOWER_9CF(), DEN_THIRD_9CF(), 0);
	...
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("den_prm_prize_money"));
	data->f225 = 1;
	SetupTVMoney(_this, prizeMoneyFile.GetInt("den_prm_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
...
void __declspec(naked) den_premier_update_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) den_premier_table_split_c()
{
	__asm
	{
		mov eax, esp
...
void setup_den_premier()
{
	...
	WriteVTablePtr(den_premier_vtable, VTableStageNews, 0x7f3080); // Scotland stage news contains champ/rel group news
	if (configFile.GetBool("showThirdPlaceInHistory", true)) WriteVTablePtr(den_premier_vtable, VTableShowThirdInHistory, 0x4110b0);
}