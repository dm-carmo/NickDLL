...
void germany_awards_default(BYTE* _this, int a2) {
...
void __declspec(naked) ger_awards_default_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		...
		push ecx
		call germany_awards_default
		add esp, 0x8
		ret 4
	}
}

char ger_awards_default_2(BYTE* _this, DWORD current_date) {
	...
	WORD nation_day = *(WORD*)(playable_leagues_table + 0x15 + sub_669340_ret * 72); //b6423d germany
...
void __declspec(naked) ger_awards_default_2_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ger_awards_default_2
		add esp, 0x8
		ret 4
	}
}

void ger_awards_team_of_week_default(BYTE* _this, DWORD award_addr) {
	DWORD* awards_list = (DWORD*)*(DWORD*)(_this + 0x4);
...
void __declspec(naked) ger_awards_team_of_week_default_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ger_awards_team_of_week_default
		add esp, 0x8
		ret 4
	}
}

void ger_awards_staff_history_default(BYTE* _this) {
	DWORD* awards_list = (DWORD*)*(DWORD*)(_this + 0x4);
...
void __declspec(naked) ger_awards_staff_history_default_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call ger_awards_staff_history_default
		add esp, 0x4
		ret
	}
}

void setup_ger_awards_default()
{
