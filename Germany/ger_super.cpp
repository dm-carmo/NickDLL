...
DWORD* ger_super_vtable = (DWORD*)0x971250;

DWORD ger_super_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 0;
...
void __declspec(naked) ger_super_fixture_caller()
{
	__asm
	{
		...
		push ecx
		call ger_super_fixtures
		add esp, 0x14
		ret 0x10
	}
}

int ger_super_teams(BYTE* _this) {
	...
	cm3_club_comps* league = get_comp(GER_FIRST_9CF());
	cm3_club_comps* cup = get_comp(GER_CUP_9CF());
...
char ger_super_update(BYTE* _this) {
	...
	data->f171 = 0;
	*((BYTE*)(_this + 0xB1)) = 0;
	ger_super_teams(_this);
	DWORD v1 = *(DWORD*)_this;
	(*(int(__thiscall**)(BYTE*))(v1 + 0x8C))(_this);
	return (*(int(__thiscall**)(BYTE*))(v1 + 0x94))(_this);
}

void __declspec(naked) ger_super_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ger_super_update
		add esp, 0x4
		ret
	}
}

void ger_super_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_518640(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = ger_super_vtable;
	data->year = year;
	data->f171 = 0;
	data->f68 = -1;
...
void setup_ger_super()
{
	...
	WriteVTablePtr(ger_super_vtable, VTableEoSUpdate, (DWORD)&ger_super_update_c);
	WriteVTablePtr(ger_super_vtable, VTableSubsRounds, 0x858e70);
}
