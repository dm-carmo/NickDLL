...
DWORD ger_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		AddPlayoffFixture(pMem, fixture_id, Date(year + 1, 5, 22), year, Friday, Evening);
...
void ger_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	...
	AddTeams(_this);
	data->prize_money_pool = add_prize_money_682F70(_this, 236810);
	data->f225 = 1;
	add_tv_money_683010(_this, 2803500, 0);
	sub_6835C0(_this);
	BYTE* ebx = 0;
