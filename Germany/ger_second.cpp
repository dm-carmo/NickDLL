...
DWORD* ger_second_vtable = (DWORD*)0x96B304;

void ger_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = ger_second_vtable;
	sub_687970(_this, 0);
...
void ger_second_free(BYTE* _this, BYTE a2) {
	ger_second_free_under(_this);
	if (a2 & 1) {
...
void __declspec(naked) ger_second_free_c()
{
	__asm
	{
		...
		push ecx
		call ger_second_free
		add esp, 0x8
		ret 4
	}
}

void ger_second_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = GER_FIRST_9CF();
	comp_data->relegates_to = GER_THIRD_9CF();
...
void __declspec(naked) ger_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ger_second_subs
		add esp, 0x4
		ret
	}
}

DWORD ger_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) ger_second_fixtures_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ger_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void ger_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
...
void ger_second_playoff_under(BYTE* _this) {
	...
	total_teams = ger_third_data->n_teams;
	table_teams = (team_league_stats*)(ger_third_data->team_league_table);
	for (int i = 0; i < total_teams; i++) {
		team_league_stats tls = table_teams[i];
...
void ger_second_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		DWORD v1 = *(DWORD*)ger_third;
		char ret = (*(int(__thiscall**)(BYTE*, int, int))(v1 + 0x10))(ger_third, 0, 1);
		if (ret != 0) {
			(*(void(__thiscall**)(BYTE*))(v1 + 0x94))(ger_third);
			current++;
			comp_data->current_stage = current;
			if (current == 0) {
				ger_second_playoff_under(_this);
			}
		}
	}
}
...
void __declspec(naked) ger_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ger_second_playoffs_c
		add esp, 0x4
		ret
	}
}

int ger_second_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage == 0) {
		...
		if (club->ClubDivision == ger_third) {
			comp_stats* ger_third_data = (comp_stats*)get_loaded_league(GER_THIRD_9CF());
			WORD num_teams = ger_third_data->n_teams;
			if (num_teams <= 0) return 0;
			team_league_stats* table = (team_league_stats*)(ger_third_data->team_league_table);
		...
		case Promoted:
			...
			return 0;
		case TopPlayoff:
			staff_history_qualified_86BDD0(staff_hist_ptr, club, (DWORD)(ger_first), None, Playoff, 0x1E);
			return 0;
		case BottomPlayoff:
...
void __declspec(naked) ger_second_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ger_second_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

char ger_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
int ger_second_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) ger_second_stage_news_c()
{
	__asm
	{
		mov eax, esp
...
void setup_ger_second()
{
	...
	WriteVTablePtr(ger_second_vtable, VTableTableFates, (DWORD)&ger_second_table_fates_c);
	WriteVTablePtr(ger_second_vtable, VTableStageNews, (DWORD)&ger_second_stage_news_c);
