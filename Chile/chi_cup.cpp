...
DWORD chi_cup_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 7) {
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, RoundOf16, 0, FixedTeamOrderInCup | NoAwayGoals, Penalties | NoAwayGoals, 4, 16, 8, 16, 0, 0, 2, 4);
		...
		FillFixtureDetails(pMem, fixture_id++, QuarterFinal, 0, FixedTeamOrderInCup | NoAwayGoals, Penalties | NoAwayGoals, 4, 8, 4, 0, 0, 0, 2, 4);
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup | NoAwayGoals, Penalties | NoAwayGoals, 6, 4, 2, 0, 0, 0, 2, 4);
...
int chi_cup_table_fates(BYTE* _this, cm3_clubs* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) chi_cup_table_fates_c()
{
	__asm
	{
		mov eax, esp
...
void chi_cup_init(BYTE* _this, WORD year, cm3_club_comps* comp) {
	...
	chi_cup_vtable->SetPointer(VTableTableFates, (DWORD)&chi_cup_table_fates_c);
	...
	chi_cup_vtable->SetPointer(VTableLoadCompInfo, 0x48CEB0);
	chi_cup_vtable->SetPointer(VTableSaveCompInfo, 0x48CEA0);
