...
void __declspec(naked) uefa_conference_league_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_conference_league_set_champion_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
DWORD uefa_conference_league_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = MetropolitanZone; // Main Path
		...
		FillFixtureDetails(pMem, fixture_id++, FirstQualifyingPhase, 4, NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 50, 25, 50, 0, 0, 2, 7, 0, 0, 67500);
		...
		FillFixtureDetails(pMem, fixture_id++, SecondQualifyingPhase, 4, FixedTeamOrderInCup3 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 86, 43, 61, 50, 0, 2, 7, 0, 0, 157500);
		...
		FillFixtureDetails(pMem, fixture_id++, ThirdQualifyingPhase, 4, FixedTeamOrderInCup3 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 52, 26, 9, 111, 0, 2, 7, 0, 0, 247500);
		...
		FillFixtureDetails(pMem, fixture_id++, Playoff, 4, FixedTeamOrderInCup3 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 38, 19, 12, 120, 0, 2, 7, 0, 0, 337500);
		...
		*stage_name_id = Phase1;
		...
		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 6, 29), year, Thursday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 7, 24), year, Thursday, Evening);
		FillFixtureDetails(pMem, fixture_id++, SecondQualifyingPhase, 4, FixedTeamOrderInCup3 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 12, 6, 12, 0, 0, 2, 7, 0, 0, 157500);

		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 7, 21), year, Monday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 8, 7), year, Thursday, Evening);
		FillFixtureDetails(pMem, fixture_id++, ThirdQualifyingPhase, 4, FixedTeamOrderInCup3 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 8, 4, 2, 12, 0, 2, 7, 0, 0, 247500);

		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 8, 4), year, Monday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 8, 21), year, Thursday, Evening);
		FillFixtureDetails(pMem, fixture_id++, Playoff, 4, FixedTeamOrderInCup3 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 10, 5, 6, 14, 0, 2, 7, 0, 0, 337500);
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, TenthRound, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 16, 8, 16, 0, 0, 2, 7, 90000);
		...
		*stage_name_id = None;
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, EleventhRound, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 16, 8, 16, 0, 0, 2, 7, 360000);
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, QuarterFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 8, 4, 0, 0, 0, 2, 7, 585000);
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 4, 2, 0, 0, 0, 2, 7, 1125000);

		AddPlayoffDrawFixture(pMem, fixture_id, Date(year + 1, 5, 8), year, Friday);
		AddPlayoffFixture(pMem, fixture_id, Date(year + 1, 5, 27), year, Wednesday, Evening, NationalStadium);
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, Final, 0, ExtraTimePenalties_1, NoTiebreak_2, 8, 2, 1, 0, 0, 0, 1, 0, 0, 3150000, 1800000);
...
void __declspec(naked) uefa_conference_league_fixture_caller()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_conference_league_reputation_setup_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_conference_league_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int uecl_583B10(BYTE* _this, BYTE* a2, int a3) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (bl > 0 && bl < 5) {
		if (club_check) {
			...
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club_check, 180000, 0, -1, 0, a2, -2);
		}
		else {
			cm3_clubs* club1 = (cm3_clubs*)*(DWORD*)(a2 + 0x1c);
			int ret = sub_5A0590(ae2a38_ptr, (BYTE*)club1);
			add_to_income_prize_money_5999A0((BYTE*)ret, 60000);
			cm3_clubs* club2 = (cm3_clubs*)*(DWORD*)(a2 + 0x20);
			ret = sub_5A0590(ae2a38_ptr, (BYTE*)club2);
			add_to_income_prize_money_5999A0((BYTE*)ret, 60000);
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club1, 60000, 0, -1, 0, a2, -2);
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club2, 60000, 0, -1, 0, a2, -2);
		}
	}
	return sub_51A150(_this, a2, a3);
}

void __declspec(naked) uecl_583B10_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int uecl_586660(BYTE* _this, DWORD* a2, char a3) {
	comp_stats* data = (comp_stats*)_this;
	if (a3 == 1) {
		...
		for (WORD i = 0; i < data->special_nteams_seedings; i++) {
			...
			if (t.club->ClubEuroFlag == data->competition_db->ClubCompID && t.f5 != 3) {
...
void __declspec(naked) uecl_586660_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_conference_league_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < group_teams; j++) {
			...
			*((DWORD*)(&pTeams[j])) = (DWORD)club;
			...
			add_to_income_prize_money_5999A0((BYTE*)ret, 1426500);
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club, 1426500, 0, -1, GroupStage, 0, -2);
		}
...
void uefa_conference_league_playoff_stage_setup(BYTE* _this) {
	...
	for (int i = 0; i < 16; i++)
	{
		...
		{
			int ret = sub_5A0590(ae2a38_ptr, (BYTE*)club);
			add_to_income_prize_money_5999A0((BYTE*)ret, 90000);
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club, 90000, 0, -1, TenthRound, 0, -2);
		}
	}
...
void uefa_conference_league_final_stage_setup(BYTE* _this) {
	...
	for (int i = 0; i < 16; i++)
	{
		...
		if (club)
		{
			int ret = sub_5A0590(ae2a38_ptr, (BYTE*)club);
			add_to_income_prize_money_5999A0((BYTE*)ret, 180000);
			add_money_to_club_from_comp_48E3E0(_this, (BYTE*)club, 180000, 0, -1, EleventhRound, 0, -2);
...
void __declspec(naked) uefa_conference_league_stages_create_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int uefa_conference_league_set_fates(BYTE* _this, cm3_clubs* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == -1 || stage == 0) {
...
void __declspec(naked) uefa_conference_league_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call uefa_conference_league_set_fates
		add esp, 0x1c
		ret 0x18
	}
}
...
void __declspec(naked) uecl_team_selection_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void setup_uefa_conference_league() {
	...
	WriteVTablePtr(uefa_conference_league_vtable, VTableStageNews, 0x48C6D0); // review? -> 584550
	WriteVTablePtr(uefa_conference_league_vtable, VTable23, (DWORD)&uecl_586660_c);
