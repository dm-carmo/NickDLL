...
DWORD uefa_champions_league_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = Phase1;
		...
		FillFixtureDetails(pMem, fixture_id++, FirstQualifyingPhase, 4, NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 28, 14, 28, 0, 0, 2, 7, 0, 0, 78750);
		...
		FillFixtureDetails(pMem, fixture_id++, SecondQualifyingPhase, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 24, 12, 10, 28, 0, 2, 7, 0, 0, 78750);
		...
		FillFixtureDetails(pMem, fixture_id++, ThirdQualifyingPhase, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 12, 6, 0, 0, 0, 2, 7, 0, 0, 78750);
		...
		FillFixtureDetails(pMem, fixture_id++, Playoff, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 10, 5, 4, 38, 0, 2, 7, 0, 0, 1900000);
		...
		*stage_name_id = Phase2;
		...
		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 6, 29), year, Thursday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 7, 23), year, Wednesday, Evening);
		FillFixtureDetails(pMem, fixture_id++, SecondQualifyingPhase, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 4, 2, 4, 0, 0, 2, 7, 0, 0, 78750);

		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 7, 21), year, Monday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 8, 6), year, Wednesday, Evening);
		FillFixtureDetails(pMem, fixture_id++, ThirdQualifyingPhase, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 8, 4, 6, 4, 0, 2, 7, 0, 0, 78750);

		AddPlayoffDrawFixture(pMem, fixture_id, Date(year, 8, 4), year, Monday);
		AddPlayoffFixture(pMem, fixture_id, Date(year, 8, 20), year, Wednesday, Evening);
		FillFixtureDetails(pMem, fixture_id++, Playoff, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 4, 2, 0, 0, 0, 2, 7, 0, 0, 1900000);
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, TenthRound, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 16, 8, 16, 0, 0, 2, 7, 0, 0, 450000);
		...
		*stage_name_id = None;
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, EleventhRound, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 16, 8, 16, 0, 0, 2, 7, 4950000);
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, QuarterFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 8, 4, 0, 0, 0, 2, 7, 5625000);
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 0, FixedTeamOrderInCup + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 4, 2, 0, 0, 0, 2, 7, 6750000);
		...
		AddPlayoffTVFixture(pMem, fixture_id, 0);
		FillFixtureDetails(pMem, fixture_id++, Final, 0, ExtraTimePenalties_1, NoTiebreak_2, 8, 2, 1, 0, 0, 0, 1, 0, 0, 11250000, 8325000);
...
char uefa_champions_league_update(BYTE* _this) {
	...
	*((BYTE*)(_this + 0xB1)) = 0;
	...
	uefa_champions_league_champs_path_teams(_this); //584990
	DWORD v1 = *(DWORD*)_this;
	...
	for (WORD j = 0; j < stage5_data->n_teams; j++) {
		...
		if (t.f6 == 1) {
			*((DWORD*)(&pTeams[playoff_idx])) = (DWORD)t.club;
			sub_9058B0((BYTE*)*uefa_seeding_list, (BYTE*)(t.club->ClubNation), 3);
			playoff_idx += 2;
		}
	}
	...
	if (stage == -1 || stage == 0) {
		...
		default:
			...
			if ((round_idx == 0 && stage == 0) || (round_idx == 2 && stage == -1)) {
				...
				staff_history_qualified_86BDD0(staff_hist_ptr, club, (DWORD)(uel_data->competition_db), MetropolitanZone,
...
void __declspec(naked) uefa_set_team_counts_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
char sub_584550(BYTE* _this, int a2, char a3, char a4, int a5, int a6, __int16 a7, int a8, char a9, int a10, LPVOID* a11) {
	// _this = comp data
	//000007EC a2
	//00       a3 => 1 if team won, 0 if they lost ? (CMP ESP + 14)
	//FF       a4 => stage idx
	//00640000 a5
	//023ECDB4 a6 => a pointer(EDI)
	//00A0     a7 => stage name id
	//00000000 a8
	//04       a9
	//00000000 a10 => ? (TEST EAX, EAX 584597)
	//023EE018 a11
	return 0;
}
...
void setup_uefa_champions_league() {
	...
	WriteVTablePtr(uefa_champions_league_vtable, VTable15, 0x48cab0); // review? -> 586fa0
	...
	WriteVTablePtr(uefa_champions_league_vtable, VTableStageNews, 0x48C6D0); // review? -> 584550
	...
	WriteDWORD(0x4B646C + 1, (DWORD)&champions_path[0]);
	WriteDWORD(0x4B8F59 + 1, (DWORD)&champions_path_short[0]);
	char* league_path = "League Path";
	char* league_path_short = "L.Path";
	WriteDWORD(0x4B6486 + 1, (DWORD)&league_path[0]);
	WriteDWORD(0x4B8F73 + 1, (DWORD)&league_path_short[0]);
}