...
vtable* ukr_premier_vtable = new vtable((BYTE*)0x969798, 0xB4);

void ukr_premier_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(ukr_premier_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void ukr_premier_free(BYTE* _this, BYTE a2) {
	ukr_premier_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) ukr_premier_free_c()
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call ukr_premier_free
		add esp, 0x8
		ret 4
	}
}

void ukr_premier_subs(BYTE* _this)
{
	...
	comp_data->relegates_to = UKR_FIRST_9CF();
...
void __declspec(naked) ukr_premier_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ukr_premier_subs
		add esp, 0x4
		ret
	}
}

DWORD ukr_premier_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	comp_stats* data = (comp_stats*)_this;
	if (stage_idx == -1) {
		if (a5)
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, Playoff, 8, FixedTeamOrderInCup, Penalties | ExtraTime, 6, 4, 2, 4, 0, 0, 2, 3);
...
void __declspec(naked) ukr_premier_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_premier_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void ukr_premier_prom_rel_update(BYTE* _this, int a2) {
	comp_stats* data = (comp_stats*)_this;
	DWORD v1 = *(DWORD*)_this;
	(*(int(__thiscall**)(BYTE*))(v1 + 0xA4))(_this);

	BYTE* ukr_first = get_loaded_league(UKR_FIRST_9CF());
	v1 = *(DWORD*)ukr_first;
	(*(int(__thiscall**)(BYTE*))(v1 + 0xA4))(ukr_first);
	process_promotion_relegation_689C80(_this, _this, ukr_first, 1, a2, -1, -1);
}

void __declspec(naked) ukr_premier_prom_rel_update_c()
{
	__asm
	{
		mov eax, esp
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_premier_prom_rel_update
		add esp, 0x8
		ret 4
	}
}

void __fastcall ukr_non_league_promotion(BYTE* _this)
{
	BYTE* comp_bytes = get_loaded_league(UKR_FIRST_9CF());
	comp_stats* comp_data = (comp_stats*)comp_bytes;

	vector<cm3_clubs*> relegated_clubs = get_relegated_teams(UKR_FIRST_9CF());
	...
	vector<cm3_clubs*> available_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_UKRAINE_9CF());
	for (size_t i = 0; i < available_clubs.size(); i++) {
		...
		if (ret_club && !is_main_club && ret_club->ClubDivision->ClubCompID != UKR_PREMIER_9CF())
		{
			available_clubs.erase(available_clubs.begin() + i);
			i--;
		}
	}
	vector<cm3_clubs*> promoted_clubs = get_random_weighted_clubs(available_clubs, 4, true);
	int i = 0;
	...
	if (promoted_teams != relegated_clubs.size()) create_message_box("Error", "Promoted and relegated club count does not match for ukr_first", false);
...
void __fastcall ukr_check_reserve_teams(BYTE* _this) {
	...
	for (WORD num = 0; num < ukr_first_data->n_teams; num++) {
		team_league_stats* table_teams = (team_league_stats*)ukr_first_data->team_league_table;
		DWORD is_main_club;
		...
		if (ret_club && !is_main_club)
		{
			...
			if (table_teams[num].league_fate != Relegated) {
				...
				if (ret_club->ClubDivision->ClubCompID == UKR_PREMIER_9CF()) {
					team_league_stats* main_club_data = get_team_league_stats(UKR_PREMIER_9CF(), ret_club);
...
void __fastcall ukr_fake_lower_relegation(BYTE* _this)
{
	vector<cm3_clubs*> lower_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_UKRAINE_9CF());
	vector<cm3_clubs*> lower_b_clubs = find_clubs_of_comp(A_LOWER_B_9CF(), NATION_UKRAINE_9CF());
	unsigned int num_to_swap = lower_clubs.size() / 4;
...
char ukr_premier_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	...
	update_club_pro_status_68A980(ukr_first, Professional, Relegated, -3, 1);
	update_club_pro_status_68A980(ukr_first, SemiProfessional, -3, Relegated, 0);

	DWORD v1 = *(DWORD*)_this;
	ukr_check_reserve_teams(_this);
	ukr_premier_prom_rel_update(_this, 1);

	ukr_non_league_promotion(_this);
	ukr_fake_lower_relegation(_this);
	...
	*((WORD*)(_this + 0xA7)) = -1;
	...
	v1 = *(DWORD*)ukr_first;
	(*(int(__thiscall**)(BYTE*))(v1 + 0x8))(ukr_first);

	sub_68AA80(_this);
	return sub_79CEE0((BYTE*)*b74340, (BYTE*)(data->competition_db));
}

void __declspec(naked) ukr_premier_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ukr_premier_update
		add esp, 0x4
		ret
	}
}

void ukr_premier_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	comp_stats* ukr_first_data = (comp_stats*)get_loaded_league(UKR_FIRST_9CF());
	total_teams = ukr_first_data->n_teams;
	table_teams = (team_league_stats*)(ukr_first_data->team_league_table);
	j = 3;
...
void ukr_premier_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		BYTE* ukr_first = get_loaded_league(UKR_FIRST_9CF());
		if (ukr_first) {
			DWORD v1 = *(DWORD*)ukr_first;
			char ret = (*(int(__thiscall**)(BYTE*, int, int))(v1 + 0x10))(ukr_first, 0, 1);
			if (ret != 0) {
				(*(void(__thiscall**)(BYTE*))(v1 + 0x94))(ukr_first);
				current++;
				comp_data->current_stage = current;
				if (current == 0) {
					ukr_premier_playoff_under(_this);
				}
			}
		}
	}
...
void __declspec(naked) ukr_premier_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ukr_premier_playoffs_c
		add esp, 0x4
		ret
	}
}

int ukr_premier_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		cm3_club_comps* ukr_first = get_comp(UKR_FIRST_9CF());
		BYTE* rounds = ((comp_stats*)(comp_data->stages[stage]))->rounds_list;
		if (club->ClubDivision == ukr_first) {
			comp_stats* mar_second_data = (comp_stats*)get_loaded_league(UKR_FIRST_9CF());
			WORD num_teams = mar_second_data->n_teams;
			if (num_teams <= 0) return 0;
			team_league_stats* table = (team_league_stats*)(mar_second_data->team_league_table);
			WORD current_round = *(WORD*)(round_data + 0x34);
			for (int i = 0; i < num_teams; i++) {
				if (table[i].club != club) continue;
				switch (fate) {
				case TopPlayoff:
					staff_history_promoted_869480(staff_hist_ptr, club, (DWORD)ukr_first, 0x32);
...
void __declspec(naked) ukr_premier_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_premier_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

void ukr_premier_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	char ret_max = max;
	if (stage == 0) {
		comp_stats* d2_comp_data = (comp_stats*)get_loaded_league(UKR_FIRST_9CF());
		cm3_clubs* club_data = (cm3_clubs*)club;
		if (club_data->ClubDivision->ClubCompID == UKR_FIRST_9CF()) {
...
void __declspec(naked) ukr_premier_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_premier_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int ukr_premier_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) ukr_premier_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_premier_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void ukr_premier_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(ukr_premier_vtable->vtable_ptr);
	ukr_premier_vtable->SetPointer(VTableSubsRounds, (DWORD)&ukr_premier_subs_c);
	ukr_premier_vtable->SetPointer(VTableInitFree, (DWORD)&ukr_premier_free_c);
	ukr_premier_vtable->SetPointer(VTableEoSUpdate, (DWORD)&ukr_premier_update_c);
	ukr_premier_vtable->SetPointer(VTableFixtures, (DWORD)&ukr_premier_fixtures_c);
	ukr_premier_vtable->SetPointer(VTablePromRelUpdate, (DWORD)&ukr_premier_prom_rel_update_c);
	ukr_premier_vtable->SetPointer(VTableReputationCalc, (DWORD)&ukr_premier_reputation_calc_c);
	ukr_premier_vtable->SetPointer(VTablePlayoffQual, (DWORD)&ukr_premier_playoffs_create);
	ukr_premier_vtable->SetPointer(VTableTableFates, (DWORD)&ukr_premier_table_fates_c);
	ukr_premier_vtable->SetPointer(VTableStageNews, (DWORD)&ukr_premier_stage_news_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) ukr_premier_vtable->SetPointer(VTableShowThirdInHistory, 0x4110b0);
	data->year = year;
	data->rules = RulesUkraine;
