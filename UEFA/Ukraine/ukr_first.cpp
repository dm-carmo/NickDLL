...
vtable* ukr_first_vtable = new vtable((BYTE*)0x96CAF4, 0xB4);

void ukr_first_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(ukr_first_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void ukr_first_free(BYTE* _this, BYTE a2) {
	ukr_first_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) ukr_first_free_c()
{
	__asm
	{
		mov eax, esp
...
void ukr_first_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = UKR_PREMIER_9CF();
...
void __declspec(naked) ukr_first_subs_c()
{
	__asm
	{
		mov eax, esp
...
DWORD ukr_first_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	comp_stats* data = (comp_stats*)_this;
	if (stage_idx == -1) {
		if (a5)
...
void __declspec(naked) ukr_first_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_first_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void ukr_first_block_promotion(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
...
char ukr_first_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
	...
	*((WORD*)(_this + 0xA7)) = -1;
	ukr_first_subs(_this);
	AddTeams(_this);
	ukr_first_block_promotion(_this);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) ukr_first_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ukr_first_update
		add esp, 0x4
		ret
	}
}

void ukr_first_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	vector<cm3_clubs*> random_lower = get_random_weighted_clubs(available_clubs, 4, true);

	j = 3;
	for (int i = 0; i < 2 && j > 0; i++)
	{
		cm3_clubs* available = random_lower[i + 2];
		*((DWORD*)(&pTeams[j])) = (DWORD)available;
		j -= 2;
	}
...
void ukr_first_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			...
			ukr_first_playoff_under(_this);
		}
	}
}

void __declspec(naked) ukr_first_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call ukr_first_playoffs_c
		add esp, 0x4
		ret
	}
}

int ukr_first_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		BYTE* rounds = ((comp_stats*)(comp_data->stages[stage]))->rounds_list;
		if (club->ClubDivision->ClubCompID == comp_data->competition_db->ClubCompID) {
		...
		cm3_club_comps* ukr_premier = get_comp(UKR_PREMIER_9CF());
		switch (fate) {
		case Champions:
		...
		case Promoted:
			...
			return 0;
		case TopPlayoff:
			staff_history_qualified_86BDD0(staff_hist_ptr, club, (DWORD)ukr_premier, None, Playoff, 0x1E);
			return 0;
		case BottomPlayoff:
...
void __declspec(naked) ukr_first_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_first_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

void ukr_first_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) ukr_first_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_first_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int ukr_first_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) ukr_first_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call ukr_first_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void ukr_first_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(ukr_first_vtable->vtable_ptr);
	ukr_first_vtable->SetPointer(VTableEoSUpdate, (DWORD)&ukr_first_update_c);
	ukr_first_vtable->SetPointer(VTableSubsRounds, (DWORD)&ukr_first_subs_c);
	ukr_first_vtable->SetPointer(VTableFixtures, (DWORD)&ukr_first_fixtures_c);
	ukr_first_vtable->SetPointer(VTableInitFree, (DWORD)&ukr_first_free_c);
	ukr_first_vtable->SetPointer(VTableReputationCalc, (DWORD)&ukr_first_reputation_calc_c);
	ukr_first_vtable->SetPointer(VTablePlayoffQual, (DWORD)&ukr_first_playoffs_create);
	ukr_first_vtable->SetPointer(VTableTableFates, (DWORD)&ukr_first_table_fates_c);
	ukr_first_vtable->SetPointer(VTableStageNews, (DWORD)&ukr_first_stage_news_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) ukr_first_vtable->SetPointer(VTableShowThirdInHistory, 0x4110b0);
	data->year = year;
