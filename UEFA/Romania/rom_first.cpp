...
vtable* rom_first_vtable = new vtable((BYTE*)0x9690C4, 0xB4);

int rom_first_7F3220(DWORD a1, DWORD a2) {
	BYTE split_pos = 8;
...
void rom_first_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(rom_first_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void rom_first_free(BYTE* _this, BYTE a2) {
	rom_first_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) rom_first_free_c()
{
	__asm
	{
		mov eax, esp
...
void rom_first_subs(BYTE* _this)
{
	...
	*((WORD*)(_this + 0xA7)) = 37; // total number of games each team will play
	...
	comp_data->relegates_to = ROM_SECOND_9CF();
...
void __declspec(naked) rom_first_subs_c()
{
	__asm
	{
		mov eax, esp
...
DWORD rom_first_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	comp_stats* data = (comp_stats*)_this;
	if (stage_idx < 0) {
		if (a5)
...
void __declspec(naked) rom_first_fixtures_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call rom_first_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void rom_first_prom_rel_update(BYTE* _this, int a2) {
	comp_stats* data = (comp_stats*)_this;
	DWORD v1 = *(DWORD*)_this;
	(*(int(__thiscall**)(BYTE*))(v1 + 0xA4))(_this);

	BYTE* rom_second = get_loaded_league(ROM_SECOND_9CF());
	v1 = *(DWORD*)rom_second;
	(*(int(__thiscall**)(BYTE*))(v1 + 0xA4))(rom_second);
	process_promotion_relegation_689C80(_this, _this, rom_second, 1, a2, -1, -1);
}

void __declspec(naked) rom_first_prom_rel_update_c()
{
	__asm
	{
		mov eax, esp
		push dword ptr[eax + 0x4]
		push ecx
		call rom_first_prom_rel_update
		add esp, 0x8
		ret 4
	}
}

void __fastcall rom_non_league_promotion(BYTE* _this)
{
	BYTE* comp_bytes = get_loaded_league(ROM_SECOND_9CF());
	comp_stats* comp_data = (comp_stats*)comp_bytes;

	vector<cm3_clubs*> relegated_clubs = get_relegated_teams(ROM_SECOND_9CF());

	vector<cm3_clubs*> available_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_ROMANIA_9CF());
...
char rom_first_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	...
	update_club_pro_status_68A980(rom_second, Professional, Relegated, -3, 1);
	update_club_pro_status_68A980(rom_second, SemiProfessional, -3, Relegated, 0);

	DWORD v1 = *(DWORD*)_this;
	rom_first_prom_rel_update(_this, 1);

	rom_non_league_promotion(_this);
	...
	*((WORD*)(_this + 0xA7)) = -1;
	rom_first_subs(_this);
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("rom_first_prize_money"));
	data->f225 = 1;
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	...
	v1 = *(DWORD*)rom_second;
	(*(int(__thiscall**)(BYTE*))(v1 + 0x8))(rom_second);

	sub_68AA80(_this);
	return sub_79CEE0((BYTE*)*b74340, (BYTE*)(data->competition_db));
}

void __declspec(naked) rom_first_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call rom_first_update
		add esp, 0x4
		ret
	}
}

void rom_first_split_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	...
	data->n_rounds = 3;
	*((DWORD*)(_this + 0xA3)) = (DWORD)&rom_first_7F3220;
}

char rom_first_table_split(BYTE* _this, DWORD current_date, int a2) {
	comp_stats* comp_data = (comp_stats*)_this;
	if (a2) {
...
void __declspec(naked) rom_first_table_split_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call rom_first_table_split
		add esp, 0xc
		ret 8
	}
}

void rom_first_playoff_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	comp_stats* rom_second_data = (comp_stats*)get_loaded_league(ROM_SECOND_9CF());
	comp_stats* prom_playoff = (comp_stats*)rom_second_data->stages[0];
...
void rom_first_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	long current = comp_data->current_stage;
	long max = comp_data->num_stages;
	if (current < max - 1) {
		BYTE* rom_second = get_loaded_league(ROM_SECOND_9CF());
		comp_stats* rom_second_data = (comp_stats*)rom_second;
		BYTE* prom_playoff = (BYTE*)rom_second_data->stages[0];
		if (prom_playoff) {
...
void __declspec(naked) rom_first_playoffs_create()
{
	__asm
	{
		mov eax, esp
		push ecx
		call rom_first_playoffs_c
		add esp, 0x4
		ret
	}
}

int rom_first_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	BYTE* staff_hist_ptr = (BYTE*)*staff_history;
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		cm3_club_comps* rom_second = get_comp(ROM_SECOND_9CF());
		BYTE* rounds = ((comp_stats*)(comp_data->stages[stage]))->rounds_list;
		if (club->ClubDivision == rom_second) {
			comp_stats* rom_second_data = (comp_stats*)get_loaded_league(ROM_SECOND_9CF());
			WORD num_teams = rom_second_data->n_teams;
			if (num_teams <= 0) return 0;
			team_league_stats* table = (team_league_stats*)(rom_second_data->team_league_table);
			WORD current_round = *(WORD*)(round_data + 0x34);
			for (int i = 0; i < num_teams; i++) {
				if (table[i].club != club) continue;
				switch (fate) {
				case TopPlayoff:
					staff_history_promoted_869480(staff_hist_ptr, club, (DWORD)rom_second, 0x32);
			...
			comp_stats* curr_stage = (comp_stats*)(rom_second_data->stages[0]);
...
void __declspec(naked) rom_first_table_fates_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call rom_first_table_fates
		add esp, 0x1c
		ret 0x18
	}
}

void rom_first_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	char ret_max = max;
	if (stage == 0) {
		comp_stats* d2_comp_data = (comp_stats*)get_loaded_league(ROM_SECOND_9CF());
		cm3_clubs* club_data = (cm3_clubs*)club;
		if (club_data->ClubDivision->ClubCompID == ROM_SECOND_9CF()) {
...
void __declspec(naked) rom_first_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call rom_first_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int rom_first_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) rom_first_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call rom_first_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void rom_first_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(rom_first_vtable->vtable_ptr);
	rom_first_vtable->SetPointer(VTableSubsRounds, (DWORD)&rom_first_subs_c);
	rom_first_vtable->SetPointer(VTableInitFree, (DWORD)&rom_first_free_c);
	rom_first_vtable->SetPointer(VTableEoSUpdate, (DWORD)&rom_first_update_c);
	rom_first_vtable->SetPointer(VTableFixtures, (DWORD)&rom_first_fixtures_c);
	rom_first_vtable->SetPointer(VTableLeagueSplit, (DWORD)&rom_first_table_split_c);
	rom_first_vtable->SetPointer(VTableReputationCalc, (DWORD)&rom_first_reputation_calc_c);
	rom_first_vtable->SetPointer(VTablePlayoffQual, (DWORD)&rom_first_playoffs_create);
	rom_first_vtable->SetPointer(VTableTableFates, (DWORD)&rom_first_table_fates_c);
	rom_first_vtable->SetPointer(VTablePromRelUpdate, (DWORD)&rom_first_prom_rel_update_c);
	rom_first_vtable->SetPointer(VTableStageNews, (DWORD)&rom_first_stage_news_c);
	if (configFile.GetBool("showThirdPlaceInHistory", true)) rom_first_vtable->SetPointer(VTableShowThirdInHistory, 0x4110b0);
	data->year = year;
