...
char* pol_second_set_champion(BYTE* _this) {
...
void __declspec(naked) pol_second_set_champion_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call pol_second_set_champion
		add esp, 0x4
		ret 0
	}
}
...
void __declspec(naked) pol_second_subs_c()
{
	__asm
	{
		mov eax, esp
...
void block_reserve_promotion_pol_second(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
char pol_second_update(BYTE* _this) {
	...
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("pol_second_prize_money"));
	data->f225 = 1;
	SetupTVMoney(_this, prizeMoneyFile.GetInt("pol_second_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	block_reserve_promotion_pol_second(_this);
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) pol_second_update_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) pol_second_free_c()
{
	__asm
	{
		mov eax, esp
...
DWORD pol_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = PromotionPlayoff;
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 8, FixedTeamOrderInCup | Penalties | ExtraTime, NoTiebreak, 6, 4, 2, 4, 0, 0, 1, 0);
		...
		FillFixtureDetails(pMem, fixture_id++, Final, 8, Penalties | ExtraTime, NoTiebreak, 6, 2, 1, 0, 0, 0, 1, 0);
...
void __declspec(naked) pol_second_fixtures_c()
{
	__asm
	{
		mov eax, esp
...
void pol_second_prom_playoffs(BYTE* _this) {
	...
	for (char i = 0; i < playoff_teams; i++) {
		seeds[team_order[i]] = playoff_teams - i;
...
void __declspec(naked) pol_second_playoffs_create()
{
	__asm
	{
		mov eax, esp
...
int pol_second_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) pol_second_table_fates_c()
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) pol_second_reputation_calc_c()
{
	__asm
	{
		mov eax, esp
...
void setup_pol_second()
{
	...
	WriteVTablePtr(pol_second_vtable, VTableTableFates, (DWORD)&pol_second_table_fates_c);
	WriteVTablePtr(pol_second_vtable, VTableSetChampion, (DWORD)&pol_second_set_champion_c);
	WriteVTablePtr(pol_second_vtable, VTableStageNews, 0x5785b0);
