...
void __declspec(naked) uefa_europa_league_set_champion_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
DWORD uefa_europa_league_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = MetropolitanZone; // Main Path
		...
		FillFixtureDetails(pMem, fixture_id++, SecondQualifyingPhase, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 18, 9, 12, 12, 0, 2, 7, 0, 0, 78750);
		...
		FillFixtureDetails(pMem, fixture_id++, ThirdQualifyingPhase, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 14, 7, 5, 24, 0, 2, 7, 0, 0, 78750);
		...
		FillFixtureDetails(pMem, fixture_id++, Playoff, 4, FixedTeamOrderInCup2 + NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 8, 24, 12, 17, 29, 0, 2, 7, 0, 0, 78750);
...
void __declspec(naked) uefa_europa_league_fixture_caller()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_europa_league_reputation_setup_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_europa_league_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int uel_583B10(BYTE* _this, BYTE* a2, int a3) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (bl > 0 && bl < 5) {
		if (club_check) {
			...
			sub_5999A0((BYTE*)ret, 202500);
			sub_48E3E0(_this, (BYTE*)club_check, 202500, 0, -1, 0, a2, -2);
		}
		else {
			cm3_clubs* club1 = (cm3_clubs*)*(DWORD*)(a2 + 0x1c);
			int ret = sub_5A0590(ae2a38_ptr, (BYTE*)club1);
			sub_5999A0((BYTE*)ret, 67500);
			cm3_clubs* club2 = (cm3_clubs*)*(DWORD*)(a2 + 0x20);
			ret = sub_5A0590(ae2a38_ptr, (BYTE*)club2);
			sub_5999A0((BYTE*)ret, 67500);
			sub_48E3E0(_this, (BYTE*)club1, 67500, 0, -1, 0, a2, -2);
			sub_48E3E0(_this, (BYTE*)club2, 67500, 0, -1, 0, a2, -2);
		}
	}
	return sub_51A150(_this, a2, a3);
}

void __declspec(naked) uel_583B10_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
int uel_586660(BYTE* _this, DWORD* a2, char a3) {
	comp_stats* data = (comp_stats*)_this;
	if (a3 == 1) {
...
void __declspec(naked) uel_586660_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_europa_league_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < group_teams; j++) {
			...
			*((DWORD*)(&pTeams[j])) = (DWORD)club;
			...
			sub_5999A0((BYTE*)ret, 1940000);
			sub_48E3E0(_this, (BYTE*)club, 1940000, 0, -1, GroupStage, 0, -2);
		}
...
void uefa_europa_league_playoff_stage_setup(BYTE* _this) {
	...
	for (int i = 0; i < 16; i++)
	{
		...
		if (club)
		{
			...
			sub_48E3E0(_this, (BYTE*)club, 135000, 0, -1, TenthRound, 0, -2);
		}
	}
...
void uefa_europa_league_final_stage_setup(BYTE* _this) {
	...
	for (int i = 0; i < 16; i++)
	{
		...
		{
			int ret = sub_5A0590(ae2a38_ptr, (BYTE*)club);
			sub_5999A0((BYTE*)ret, 270000);
			sub_48E3E0(_this, (BYTE*)club, 270000, 0, -1, EleventhRound, 0, -2);
...
void __declspec(naked) uefa_europa_league_stages_create_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uefa_europa_league_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uel_team_selection_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) uel_48CAB0_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void setup_uefa_europa_league() {
	...
	WriteVTablePtr(uefa_europa_league_vtable, VTableStageNews, 0x48C6D0); // review? -> 584550
	WriteVTablePtr(uefa_europa_league_vtable, VTable23, (DWORD)&uel_586660_c);
