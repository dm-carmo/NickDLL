...
DWORD CreateJapanLeagueCupFixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 0;
		...
		*stage_name_id = None;
		...
		FillFixtureDetails(pMem, fixture_id++, SemiFinal, 1, NoTiebreak_1, ExtraTimePenaltiesNoAwayGoals_2, 6, 4, 2, 0, 0, 0, 2, 4, 45000, 0, 0);
...
void __declspec(naked) jpn_league_cup_fixture_caller()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		...
		push ecx
		call CreateJapanLeagueCupFixtures
		add esp, 0x14
		ret 0x10
	}
}
