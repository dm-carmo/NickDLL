...
DWORD* jpn_second_vtable = (DWORD*)0x96CAF4;

void jpn_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = jpn_second_vtable;
	DWORD x = 0;
	sub_687970(_this, 0);
...
void jpn_second_free(BYTE* _this, BYTE a2) {
	jpn_second_free_under(_this);
	if (a2 & 1) {
		sub_944C94_free(_this);
	}
}

void __declspec(naked) jpn_second_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push ecx
		call jpn_second_free
		add esp, 0x8
		ret 4
	}
}

void jpn_second_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = JPN_FIRST_9CF();
	comp_data->relegates_to = JPN_THIRD_9CF();
...
void __declspec(naked) jpn_second_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call jpn_second_subs
		add esp, 0x4
		ret
	}
}

DWORD jpn_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) jpn_second_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call jpn_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void jpn_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	...
	AddTeams(_this);
	data->prize_money_pool = add_prize_money_682F70(_this, 2475);
	data->f225 = 1;
	add_tv_money_683010(_this, 985815, 0);
	sub_6835C0(_this);
	BYTE* ebx = 0;
...
void jpn_second_playoffs_under(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
...
void jpn_second_playoffs_create_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) jpn_second_playoffs_under(_this);
	}
}

void __declspec(naked) jpn_second_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call jpn_second_playoffs_create_c
		add esp, 0x4
		ret
	}
}
...
void __declspec(naked) jpn_second_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void jpn_second_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) jpn_second_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call jpn_second_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

char jpn_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	BYTE* ebx = 0;
	data->f76 = 0;
...
void __declspec(naked) jpn_second_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
		push ecx
		call jpn_second_update
		add esp, 0x4
		ret
	}
}

void setup_jpn_second()
{
	WriteVTablePtr(jpn_second_vtable, VTableSubsRounds, (DWORD)&jpn_second_subs_c);
	WriteVTablePtr(jpn_second_vtable, VTableFixtures, (DWORD)&jpn_second_fixtures_c);
