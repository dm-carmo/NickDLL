...
void __declspec(naked) sco_league_2_table_fates_c()
{
	__asm
	{
		mov eax, esp
...
int sco_league_2_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) sco_league_2_stage_news_c()
{
	__asm
	{
		mov eax, esp
...
void setup_sco_league_2()
{
	...
	WriteVTablePtr(sco_league_2_vtable, VTableTableFates, (DWORD)&sco_league_2_table_fates_c);
	WriteVTablePtr(sco_league_2_vtable, VTableStageNews, (DWORD)&sco_league_2_stage_news_c);
