...
DWORD sco_lowland_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 1) {
		...
		*stage_name_id = West + stage_idx;
		...
		if (numberOfLeagueTeams > 16) AddFixtureNoTV(pMem, fixture_id++, Date(year, 8, 19), year, Tuesday, Evening);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 8, 23), year, Saturday);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 8, 30), year, Saturday);
		if (numberOfLeagueTeams > 16) AddFixtureNoTV(pMem, fixture_id++, Date(year, 9, 2), year, Tuesday, Evening);
		...
		if (numberOfLeagueTeams > 16) AddFixtureNoTV(pMem, fixture_id++, Date(year, 11, 4), year, Tuesday, Evening);
		...
		if (numberOfLeagueTeams > 16) AddFixtureNoTV(pMem, fixture_id++, Date(year, 12, 9), year, Tuesday, Evening);
