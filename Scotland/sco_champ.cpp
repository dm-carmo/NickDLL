...
DWORD* sco_champ_vtable = (DWORD*)0x96F4B4;

void sco_champ_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = sco_champ_vtable;
	sub_687970(_this, 0);
...
void sco_champ_free(BYTE* _this, BYTE a2) {
	sco_champ_free_under(_this);
	if (a2 & 1) {
...
void sco_champ_subs(BYTE* _this)
{
	...
	comp_data->promotes_to = SCO_PREMIER_9CF();
	comp_data->relegates_to = SCO_LEAGUE_1_9CF();
...
DWORD sco_champ_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		if (a5)
			*a5 = 1;
...
void __declspec(naked) sco_champ_fixtures_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call sco_champ_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void sco_champ_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	...
	sco_champ_subs(_this);
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("sco_champ_prize_money"));
	data->f225 = 1;
	SetupTVMoney(_this, prizeMoneyFile.GetInt("sco_champ_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
...
void sco_champ_playoff_rele(BYTE* _this) {
	char stage_num = 1;
	comp_stats* comp_data = (comp_stats*)_this;
	...
	DWORD v1 = *(DWORD*)sco_league_1;
	char ret = (*(int(__thiscall**)(BYTE*, int, int))(v1 + 0x10))(sco_league_1, 0, 1);
	if (ret != 0) {
		comp_stats* sco_league_1_data = (comp_stats*)sco_league_1;
		total_teams = sco_league_1_data->n_teams;
		table_teams = (team_league_stats*)(sco_league_1_data->team_league_table);
		WORD start_idx = 3;
		for (int i = 0; i < total_teams; i++) {
			team_league_stats tls = table_teams[i];
...
void sco_champ_playoff_prom(BYTE* _this) {
	char stage_num = 0;
	comp_stats* comp_data = (comp_stats*)_this;
...
void sco_champ_playoffs_c(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 0) {
			...
			sco_champ_playoff_prom(_this);
			sco_champ_playoff_rele(_this);
		}
	}
}
...
int sco_champ_table_indicators(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	comp_stats* comp_data = (comp_stats*)_this;
	if (stage == 0) {
		cm3_club_comps* sco_premier = get_comp(SCO_PREMIER_9CF());
		...
		if (club->ClubDivision == sco_league_1) {
			...
			team_league_stats* table = (team_league_stats*)(sco_league_1_data->team_league_table);
			for (int i = 0; i < num_teams; i++) {
				if (table[i].club != club) continue;
				switch (fate) {
				case TopPlayoff:
					staff_history_promoted_869480(staff_hist_ptr, club, (DWORD)sco_league_1, 0x32);
...
char sco_champ_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
...
void __declspec(naked) sco_champ_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call sco_champ_update
		add esp, 0x4
		ret
	}
}

void sco_champ_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (stage == 0) {
		...
		comp_stats* d3_comp_data = (comp_stats*)get_loaded_league(SCO_LEAGUE_1_9CF());
		cm3_clubs* club_data = (cm3_clubs*)club;
		if (club_data->ClubDivision->ClubCompID == SCO_LEAGUE_1_9CF()) {
...
void __declspec(naked) sco_champ_reputation_calc_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call sco_champ_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

int sco_champ_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
	...
	if (stage_id == -1) return sub_48C6D0(_this, club_idx, fate, stage_id, stage_name_idx, round_data, a7, 0, a9, show_body_text, ret_str_ptr);
		...
		if (fate == TopPlayoff && !show_body_text) {
			cm3_club_comps* upper_comp = get_comp(SCO_PREMIER_9CF());
...
void __declspec(naked) sco_champ_stage_news_c()
{
	__asm
	{
		mov eax, esp
		...
		push dword ptr[eax + 0x4]
		push ecx
		call sco_champ_stage_news
		add esp, 0x2c
		ret 0x28
	}
}

void setup_sco_champ()
{
	WriteVTablePtr(sco_champ_vtable, VTableSubsRounds, (DWORD)&sco_champ_subs_c);
	WriteVTablePtr(sco_champ_vtable, VTableInitFree, (DWORD)&sco_champ_free_c);
	WriteVTablePtr(sco_champ_vtable, VTableEoSUpdate, (DWORD)&sco_champ_update_c);
