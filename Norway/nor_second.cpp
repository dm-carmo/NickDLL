...
vtable* nor_second_vtable = new vtable((BYTE*)0x9702A0, 0xB4);
...
void __declspec(naked) nor_second_set_champion_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nor_second_set_champion
		add esp, 0x4
		ret 0
	}
}

void nor_second_free_under(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->comp_vtable = (DWORD*)(nor_second_vtable->vtable_ptr);
	sub_687970(_this, 0);
...
void nor_second_free(BYTE* _this, BYTE a2) {
	nor_second_free_under(_this);
	if (a2 & 1) {
...
void __declspec(naked) nor_second_free_c()
{
	__asm
	{
		...
		push ecx
		call nor_second_free
		add esp, 0x8
		ret 4
	}
}

void nor_second_subs(BYTE* _this)
{
	...
	if ((selected & 4) == 0) {
		...
		comp_data->relegates_to = NOR_THIRD_9CF();
	}
...
void __declspec(naked) nor_second_subs_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nor_second_subs
		add esp, 0x4
		ret
	}
}

DWORD nor_second_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx < 1) {
		if (a5)
			*a5 = 1;
		...
		*stage_name_id = NumericGroupStage + stage_idx;
		...
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		tv_id = 0;
		AddFixture(pMem, fixture_id, Date(year, 4, 21), year, Monday);
		...
		AddFixtureTV(pMem, fixture_id, tv_id++, 2, Saturday, Afternoon);
		AddFixtureTV(pMem, fixture_id++, tv_id++);
		AddFixtureNoTV(pMem, fixture_id++, Date(year, 10, 25), year, Sunday);
...
void __declspec(naked) nor_second_fixtures_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nor_second_fixtures
		add esp, 0x14
		ret 0x10
	}
}

void nor_second_setup_groups(BYTE* _this, BYTE idx) {
	DWORD v1 = *(DWORD*)_this;
	WORD num_rounds = 0;
	WORD stage_name_id = 0;
	BYTE* pFixtures = (BYTE*)(*(int(__thiscall**)(BYTE*, int, WORD*, WORD*, DWORD))(v1 + 0x3C))(_this, idx, &num_rounds, &stage_name_id, 0);
	comp_stats* data = (comp_stats*)_this;
	DWORD group_id = NOR_SECOND_G2_9CF();
	...
	SetupTVMoney(pStage, prizeMoneyFile.GetInt("nor_second_tv_money"), 0);
...
void nor_second_reputation_setup(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) nor_second_reputation_setup_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nor_second_reputation_setup
		add esp, 0x4
		ret
	}
}

void block_reserve_promotion_nor_second(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	comp_stats* curr_stage = comp_data;
	for (char al = -1; al < 1; al++) {
		if (al >= 0) {
...
char nor_second_update(BYTE* _this) {
	comp_stats* data = (comp_stats*)_this;
	data->f76 = 0;
	sub_687970(_this, 0);
	if (data->fixtures_table) {
	...
	SetupTVMoney(_this, prizeMoneyFile.GetInt("nor_second_tv_money"), 0);
	sub_6827D0(_this, 0);
	sub_6835C0(_this);
	for (BYTE i = 0; i < 1; i++) {
		nor_second_setup_groups(_this, i);
	}
	block_reserve_promotion_nor_second(_this);
	DWORD v1 = *(DWORD*)_this;
...
void __declspec(naked) nor_second_update_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nor_second_update
		add esp, 0x4
		ret
	}
}

void nor_second_playoffs_under(BYTE* _this) {
	char stage_num = 1;
	comp_stats* comp_data = (comp_stats*)_this;
...
void nor_second_playoffs_create(BYTE* _this) {
	comp_stats* comp_data = (comp_stats*)_this;
	...
	if (current < max - 1) {
		...
		if (current == 1) {
			...
			nor_second_playoffs_under(_this);
		}
	}
}
...
void __declspec(naked) nor_second_playoffs_create_c()
{
	__asm
	{
		mov eax, esp
		push ecx
		call nor_second_playoffs_create
		add esp, 0x4
		ret
	}
}
...
int nor_second_table_indicators(BYTE* _this, cm3_clubs* club, char fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
	...
	if (stage < 1) {
		...
		comp_stats* playoff_stage = (comp_stats*)(nor_first_data->stages[1]);
		for (char al = -1; al < 1; al++) {
			if (al >= 0) {
				curr_stage = (comp_stats*)(comp_data->stages[al]);
			}
			...
			for (int i = 0; i < num_teams; i++) {
				...
				case TopPlayoff:
					...
					*a5 = 1;
					if (playoff_stage)
					{
						((teams_seeded*)playoff_stage->teams_list)[1].club = (cm3_clubs*)club;
						nor_first_data->current_stage = 1;
					}
					return 0;
				case Promoted:
...
void nor_second_awards(BYTE* _this, DWORD** team_list, WORD* total_teams) {
	*total_teams = 28;
...
void __declspec(naked) nor_second_awards_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nor_second_awards
		add esp, 0xc
		ret 8
	}
}

void nor_second_reputation_calc(BYTE* _this, BYTE* club, char stage, char current, char min, char max) {
	comp_stats* comp_data = (comp_stats*)_this;
...
void __declspec(naked) nor_second_reputation_calc_c()
{
	__asm
	{
		...
		push dword ptr[eax + 0x4]
		push ecx
		call nor_second_reputation_calc
		add esp, 0x18
		ret 0x14
	}
}

void nor_second_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	sub_682200(_this);
	comp_stats* data = (comp_stats*)_this;
	data->competition_db = comp;
	data->comp_vtable = (DWORD*)(nor_second_vtable->vtable_ptr);
	nor_second_vtable->SetPointer(VTableInitFree, (DWORD)&nor_second_free_c);
	nor_second_vtable->SetPointer(VTableEoSUpdate, (DWORD)&nor_second_update_c);
	nor_second_vtable->SetPointer(VTableFixtures, (DWORD)&nor_second_fixtures_c);
	nor_second_vtable->SetPointer(VTableReputationSetup, (DWORD)&nor_second_reputation_setup_c);
	...
	nor_second_vtable->SetPointer(VTableLoadCompInfo, 0x48ceb0);
	nor_second_vtable->SetPointer(VTableSaveCompInfo, 0x48cea0);
	data->year = year;
	...
	block_reserve_promotion_nor_second(_this);
	nor_second_reputation_setup(_this);
}

void setup_nor_second()
{
}