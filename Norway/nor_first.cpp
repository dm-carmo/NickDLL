...
void __declspec(naked) nor_first_free_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) nor_first_subs_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) nor_first_fixtures_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void nor_first_init(BYTE* _this, WORD year, cm3_club_comps* comp)
{
	...
	AddTeams(_this);
	data->prize_money_pool = add_prize_money_682F70(_this, 15500);
	data->f225 = 1;
	add_tv_money_683010(_this, 72538, 0);
	sub_6835C0(_this);
	BYTE* ebx = 0;
...
void __declspec(naked) nor_first_playoffs_create()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) nor_first_set_table_fate()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) nor_first_update_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
...
void __declspec(naked) nor_first_reputation_calc_c()		// used as a __thiscall -> __cdecl converter
{
	__asm
	{
		mov eax, esp
