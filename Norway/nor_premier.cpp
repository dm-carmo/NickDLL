...
void nor_premier_prom_rel_update(BYTE* _this, int a2) {
	...
	process_promotion_relegation_689C80(_this, _this, nor_first, 1, a2, -1, -1);
	...
	process_promotion_relegation_689C80(_this, nor_first, nor_second, 1, a2, -1, -1);
	process_promotion_relegation_689C80(_this, nor_first, nor_second_grp, 1, a2, -1, -1);
	...
	if (nor_third) {
		...
		process_promotion_relegation_689C80(_this, nor_second, nor_third, 1, a2, -1, -1);
		for (int i = 0; i < 5; i++)
		{
			BYTE* nor_third_grp = (BYTE*)nor_third_data->stages[i];
			process_promotion_relegation_689C80(_this, nor_second, nor_third_grp, 1, a2, -1, -1);
		}

		process_promotion_relegation_689C80(_this, nor_second_grp, nor_third, 1, a2, -1, -1);
		for (int i = 0; i < 5; i++)
		{
			BYTE* nor_third_grp = (BYTE*)nor_third_data->stages[i];
			process_promotion_relegation_689C80(_this, nor_second_grp, nor_third_grp, 1, a2, -1, -1);
		}
	}
}
...
void __fastcall fake_nor_non_league_promotion(BYTE* _this)
{
	vector<cm3_clubs*> d3_clubs = find_clubs_of_comp(NOR_THIRD_9CF(), NATION_NORWAY_9CF());
	vector<cm3_clubs*> lower_clubs = find_clubs_of_comp(A_LOWER_9CF(), NATION_NORWAY_9CF());

	vector<cm3_clubs*> promoted_clubs = get_random_weighted_clubs(lower_clubs, 18, true);
	vector<cm3_clubs*> relegated_clubs = get_random_weighted_clubs(d3_clubs, 18, false);
...
char nor_premier_update(BYTE* _this) {
	...
	update_club_pro_status_68A980(nor_first, SemiProfessional, Relegated, -3, 1);
	update_club_pro_status_68A980(nor_first, SemiProfessional, -3, Relegated, 0);
	// All teams that were not relegated from D3 must be semi-professional
	comp_stats* nor_second_data = (comp_stats*)nor_second;
	BYTE* nor_second_grp = (BYTE*)nor_second_data->stages[0];
	update_club_pro_status_68A980(nor_second, SemiProfessional, Relegated, -3, 1);
	update_club_pro_status_68A980(nor_second, SemiProfessional, Relegated, -3, 0);
	update_club_pro_status_68A980(nor_second_grp, SemiProfessional, Relegated, -3, 1);
	update_club_pro_status_68A980(nor_second_grp, SemiProfessional, Relegated, -3, 0);
	if (nor_third) {
		comp_stats* nor_third_data = (comp_stats*)nor_third;
		// All teams that were relegated from D4 must be amateur
		update_club_pro_status_68A980(nor_third, Amateur, -3, Relegated, 0);
		for (int i = 0; i < 5; i++)
		{
			BYTE* nor_third_grp = (BYTE*)nor_third_data->stages[i];
			update_club_pro_status_68A980(nor_third_grp, Amateur, -3, Relegated, 0);
		}
	}
	...
	if (nor_third) {
		...
		fake_nor_non_league_promotion(_this);
	}
	...
	AddTeams(_this);
	data->prize_money_pool = SetupPrizeMoney(_this, prizeMoneyFile.GetInt("nor_prm_prize_money"));
	data->f225 = 1;
	SetupTVMoney(_this, prizeMoneyFile.GetInt("nor_prm_tv_money"), 0);
	sub_6835C0(_this);
	sub_6827D0(_this, 0);
	DWORD v1 = *(DWORD*)_this;
...
DWORD nor_premier_fixtures(BYTE* _this, char stage_idx, WORD* num_rounds, WORD* stage_name_id, DWORD* a5)
{
	if (stage_idx == -1) {
		...
		*stage_name_id = None;
		...
		AddPlayoffFixture(pMem, fixture_id, Date(year, 12, 18), year, Thursday, Evening);
...
int nor_premier_table_fates(BYTE* _this, cm3_clubs* club, BYTE fate, char stage, BYTE* a5, BYTE* round_data, int a7) {
...
void __declspec(naked) nor_premier_table_fates_c()
{
	__asm
	{
		mov eax, esp
...
int nor_premier_stage_news(BYTE* _this, int club_idx, char fate, char stage_id, int stage_name_idx, int round_data, __int16 a7, int a8, char a9, int show_body_text, LPVOID* ret_str_ptr) {
	comp_stats* data = (comp_stats*)_this;
...
void __declspec(naked) nor_premier_stage_news_c()
{
	__asm
	{
		mov eax, esp
...
void setup_nor_premier()
{
	...
	WriteVTablePtr(nor_premier_vtable, VTableTableFates, (DWORD)&nor_premier_table_fates_c);
	WriteVTablePtr(nor_premier_vtable, VTablePromRelUpdate, (DWORD)&nor_premier_prom_rel_update_c);
	WriteVTablePtr(nor_premier_vtable, VTableStageNews, (DWORD)&nor_premier_stage_news_c);
